...
import static org.wso2.carbon.identity.core.dao.SAMLSSOServiceProviderConstants.SAML2TableColumns.DEFAULT_FLAG;
...
public class SAMLSSOServiceProviderDAOImpl implements SAMLSSOServiceProviderDAO {
    ...
    public boolean addServiceProvider(SAMLSSOServiceProviderDO serviceProviderDO) throws IdentityException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getDBConnection(true)) {
            try {
                ...
                processAddCustomAttributes(serviceProviderId, connection, serviceProviderDO);
    ...
    private void processAddServiceProvider(Connection connection, SAMLSSOServiceProviderDO serviceProviderDO)
        ...
        try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
            ...
            statement.setString(REQ_SIG_VALIDATION, serviceProviderDO.isDoValidateSignatureInRequests() ? "1" : "0");
            statement.setString(SIGN_RESPONSE, serviceProviderDO.isDoSignResponse() ? "1" : "0");
            ...
            statement.setString(ENCRYPT_ASSERTION, serviceProviderDO.isDoEnableEncryptedAssertion() ? "1" : "0");
            ...
            statement.setString(ATTR_PROFILE_ENABLED, serviceProviderDO.isEnableAttributesByDefault() ? "1" : "0");
            statement.setString(ATTR_SERVICE_INDEX, serviceProviderDO.getAttributeConsumingServiceIndex());
            statement.setString(SLO_PROFILE_ENABLED, serviceProviderDO.isDoSingleLogout() ? "1" : "0");
            ...
            statement.setString(IDP_INIT_SSO_ENABLED, serviceProviderDO.isIdPInitSSOEnabled() ? "1" : "0");
            statement.setString(IDP_INIT_SLO_ENABLED, serviceProviderDO.isIdPInitSLOEnabled() ? "1" : "0");
            statement.setString(QUERY_REQUEST_PROFILE_ENABLED,
                    serviceProviderDO.isAssertionQueryRequestProfileEnabled() ? "1" : "0");
            statement.setString(ECP_ENABLED, serviceProviderDO.isSamlECP() ? "1" : "0");
            statement.setString(ARTIFACT_BINDING_ENABLED, serviceProviderDO.isEnableSAML2ArtifactBinding() ? "1" : "0");
            statement.setString(ARTIFACT_RESOLVE_REQ_SIG_VALIDATION,
                    serviceProviderDO.isDoValidateSignatureInArtifactResolve() ? "1" : "0");
    ...
    private void processAddCustomAttributes(int serviceProviderId, Connection connection,
        ...
        try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
            ...
            for (ConfigTuple customAttribute : customAttributes) {
                ...
                statement.setString(DEFAULT_FLAG,
                        isDefaultAssertionConsumerUrl(serviceProviderDO, key, value) ? "1" : "0");
                statement.setInt(TENANT_ID, tenantId);
                statement.addBatch();
            }
    ...
    private List<ConfigTuple> processGetCustomAttributes(Connection connection, int serviceProviderId,
        ...
        try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    if (StringUtils.equals("1", resultSet.getString(DEFAULT_FLAG))) {
                        serviceProviderDO.setDefaultAssertionConsumerUrl(value);
                    }
                }
            }
        }
    ...
    private SAMLSSOServiceProviderDO resourceToObject(ResultSet resultSet) throws SQLException {
        ...
        serviceProviderDO.setDoValidateSignatureInRequests(getBooleanValue(resultSet.getString(REQ_SIG_VALIDATION)));
        serviceProviderDO.setDoSignResponse(getBooleanValue(resultSet.getString(SIGN_RESPONSE)));
        ...
        serviceProviderDO.setDoEnableEncryptedAssertion(getBooleanValue(resultSet.getString(ENCRYPT_ASSERTION)));
        ...
        serviceProviderDO.setEnableAttributesByDefault(getBooleanValue(resultSet.getString(ATTR_PROFILE_ENABLED)));
        serviceProviderDO.setAttributeConsumingServiceIndex(resultSet.getString(ATTR_SERVICE_INDEX));
        serviceProviderDO.setDoSingleLogout(getBooleanValue(resultSet.getString(SLO_PROFILE_ENABLED)));
        ...
        serviceProviderDO.setIdPInitSSOEnabled(getBooleanValue(resultSet.getString(IDP_INIT_SSO_ENABLED)));
        serviceProviderDO.setIdPInitSLOEnabled(getBooleanValue(resultSet.getString(IDP_INIT_SLO_ENABLED)));
        serviceProviderDO.setAssertionQueryRequestProfileEnabled(
                getBooleanValue(resultSet.getString(QUERY_REQUEST_PROFILE_ENABLED)));
        serviceProviderDO.setSamlECP(getBooleanValue(resultSet.getString(ECP_ENABLED)));
        serviceProviderDO.setEnableSAML2ArtifactBinding(getBooleanValue(resultSet.getString(ARTIFACT_BINDING_ENABLED)));
        serviceProviderDO.setDoValidateSignatureInArtifactResolve(
                getBooleanValue(resultSet.getString(ARTIFACT_RESOLVE_REQ_SIG_VALIDATION)));
...
//        serviceProviderDO.setTenantId(resultSet.getInt(TENANT_ID));
    ...
    private boolean getBooleanValue(String booleanValueAsString) {

        return "1".equals(booleanValueAsString);
    }

    private boolean isDefaultAssertionConsumerUrl(SAMLSSOServiceProviderDO serviceProviderDO, String key,
                                                  String value) {

        return StringUtils.equals(key, IdentityRegistryResources.PROP_SAML_SSO_ASSERTION_CONS_URLS) &&
                StringUtils.equals(value, serviceProviderDO.getDefaultAssertionConsumerUrl());
    }
