...
public class SQLConstants {
    ...
    public static final String DELETE_CLAIM_PROPERTY = "DELETE FROM IDN_CLAIM_PROPERTY WHERE LOCAL_CLAIM_ID=? AND " +
            "TENANT_ID=?";
    public static final String UPDATE_CLAIM_PROPERTY = "UPDATE IDN_CLAIM_PROPERTY SET PROPERTY_NAME = ?, " + 
            "PROPERTY_VALUE = ? WHERE LOCAL_CLAIM_ID = ? AND PROPERTY_NAME = ? AND TENANT_ID = ?";
    public static final String DELETE_CLAIM_PROPERTY_BY_NAME = "DELETE FROM IDN_CLAIM_PROPERTY WHERE LOCAL_CLAIM_ID=? " +
            "AND PROPERTY_NAME=? AND TENANT_ID=?";
    ...
    public static final String IS_LOCAL_CLAIM_MAPPED_EX_DIALECT = "SELECT MAPPED_LOCAL_CLAIM_ID FROM " +
            "IDN_CLAIM_MAPPING WHERE MAPPED_LOCAL_CLAIM_ID=(SELECT ID FROM IDN_CLAIM WHERE DIALECT_ID=(SELECT ID FROM" +
            " IDN_CLAIM_DIALECT WHERE DIALECT_URI=? AND TENANT_ID=?) AND CLAIM_URI=? AND TENANT_ID=?) AND TENANT_ID=?" +
            " AND EXT_CLAIM_ID IN (SELECT ID FROM IDN_CLAIM WHERE DIALECT_ID=(SELECT ID FROM IDN_CLAIM_DIALECT WHERE "
            + "DIALECT_URI=? AND TENANT_ID=?))";
    ...
    public static final String FETCH_EXTERNAL_MAPPED_CLAIM_OF_LOCAL_CLAIM = "SELECT IDN_CLAIM.CLAIM_URI, " +
            "IDN_CLAIM_DIALECT.DIALECT_URI FROM IDN_CLAIM_DIALECT INNER JOIN IDN_CLAIM ON IDN_CLAIM.DIALECT_ID = " +
            "IDN_CLAIM_DIALECT.ID WHERE IDN_CLAIM.ID IN (SELECT IDN_CLAIM_MAPPING.EXT_CLAIM_ID FROM " +
            "IDN_CLAIM_MAPPING INNER JOIN IDN_CLAIM ON IDN_CLAIM.ID  = IDN_CLAIM_MAPPING.MAPPED_LOCAL_CLAIM_ID " +
            "WHERE IDN_CLAIM.CLAIM_URI = ? AND IDN_CLAIM.TENANT_ID = ? )";
}
