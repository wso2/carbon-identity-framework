...
public class SQLConstants {
    ...
    public static final String GET_MAPPED_ATTRIBUTES = "SELECT IDN_CLAIM_MAPPED_ATTRIBUTE.USER_STORE_DOMAIN_NAME, " +
            "IDN_CLAIM_MAPPED_ATTRIBUTE.ATTRIBUTE_NAME, IDN_CLAIM_MAPPED_ATTRIBUTE.LOCAL_CLAIM_ID " +
            "FROM IDN_CLAIM_MAPPED_ATTRIBUTE " +
            "INNER JOIN IDN_CLAIM ON IDN_CLAIM_MAPPED_ATTRIBUTE.LOCAL_CLAIM_ID = IDN_CLAIM.ID " +
            "INNER JOIN IDN_CLAIM_DIALECT ON IDN_CLAIM.DIALECT_ID = IDN_CLAIM_DIALECT.ID " +
            "WHERE IDN_CLAIM_DIALECT.DIALECT_URI=? AND IDN_CLAIM_DIALECT.TENANT_ID=? AND IDN_CLAIM.TENANT_ID=? " +
            "AND IDN_CLAIM_MAPPED_ATTRIBUTE.TENANT_ID=? " +
            "ORDER BY IDN_CLAIM_MAPPED_ATTRIBUTE.LOCAL_CLAIM_ID";
    public static final String GET_CLAIMS_PROPERTIES_QUERY = "SELECT IDN_CLAIM_PROPERTY.PROPERTY_NAME, " +
            "IDN_CLAIM_PROPERTY.PROPERTY_VALUE, IDN_CLAIM_PROPERTY.LOCAL_CLAIM_ID FROM IDN_CLAIM_PROPERTY " +
            "INNER JOIN IDN_CLAIM ON IDN_CLAIM_PROPERTY.LOCAL_CLAIM_ID = IDN_CLAIM.ID " +
            "INNER JOIN IDN_CLAIM_DIALECT ON IDN_CLAIM.DIALECT_ID = IDN_CLAIM_DIALECT.ID " +
            "AND IDN_CLAIM_DIALECT.DIALECT_URI=? AND IDN_CLAIM_DIALECT.TENANT_ID=? " +
