...
import org.wso2.carbon.identity.debug.framework.core.extension.DebugProcessor;
...
public class DebugProtocolRouter {
    ...
    public enum DebugProtocolType {
         ...
         *
         * @param componentType Component type (e.g., "ContextProvider", "Executer",
         *                      "DebugProcessor").
     ...
     * @return Detected DebugProtocolType, defaults to OAUTH2_OIDC if detection
     *         fails.
     */
    ...
    /**
     * Detects protocol type for a resource ID with fallback to OAuth2/OIDC on
     * error.
     * Loads the resource configuration and identifies the authenticator type.
     * Provides comprehensive error handling for resource loading and configuration
     * issues.
     *
     * @param resourceId Resource ID or name.
     * @return Detected DebugProtocolType, defaults to OAUTH2_OIDC if detection
     ...
     * Handles NullPointerException that may occur due to corrupted resource
     * configuration.
     *
     * @param resource   The resource.
     * @param resourceId The resource ID (used for logging).
     * @return Detected DebugProtocolType, defaults to OAUTH2_OIDC if corruption is
     *         detected.
     */
    ...
    /**
     * Detects protocol type from resource configuration by analyzing authenticator
     * names.
     * Handles cases where resource authenticator configs may be null due to data
     * corruption.
     *
    ...
    private static DebugProtocolType detectProtocolFromAuthenticator(String authenticatorName) {
        ...
        // Check for OIDC/OAuth2 authenticators (must be after others since it's the
        // fallback)
     ...
     *
     * @param resourceId Resource ID.
     * @return DebugExecutor for the resource, or null if not available or not
     *         applicable.
     */
    ...
    /**
     * Normalizes the resource type by converting to uppercase and trimming
     * whitespace.
     *
    ...
    /**
     * Checks if the normalized resource type represents an identity provider
     * resource.
     *
    ...
    private static Object createIdpDebugHandler() {
        ...
        // Get IDP handler from framework service data holder (registered by IDP module)
        DebugResourceHandler idpHandler = DebugFrameworkServiceDataHolder.getIdpDebugResourceHandler();
        if (idpHandler == null) {
