...
public class CacheBackedTopicManagementDAO implements TopicManagementDAO {
     ...
     */
    @Override
    public void addTopic(String topic, int tenantId) throws TopicManagementException {

        topicManagementDAO.addTopic(topic, tenantId);
    }
    ...
    public boolean isTopicExists(String topic, int tenantId) throws TopicManagementException {
        ...
        if (cacheEntry != null) {
            LOG.debug(
                    "Topic cache hit for topic: " + topic + " for tenant ID: " + tenantId + ". Returning from cache.");
            return true;
        }
