...
public class IdPManagementConstants {
    ...
    public static class SQLQueries {
        ...
        public static final String GET_CONNECTED_APPS_ORACLE = "SELECT UUID FROM (SELECT UUID FROM (SP_AUTH_STEP " +
                "INNER JOIN SP_FEDERATED_IDP ON SP_AUTH_STEP.ID=SP_FEDERATED_IDP.ID) INNER JOIN SP_APP ON " +
                "SP_AUTH_STEP.APP_ID=SP_APP.ID WHERE AUTHENTICATOR_ID IN (SELECT ID FROM IDP_AUTHENTICATOR WHERE " +
                "IDP_ID = (SELECT ID FROM IDP WHERE UUID = ?))) WHERE ROWNUM <= ? AND ROWNUM > ?";
