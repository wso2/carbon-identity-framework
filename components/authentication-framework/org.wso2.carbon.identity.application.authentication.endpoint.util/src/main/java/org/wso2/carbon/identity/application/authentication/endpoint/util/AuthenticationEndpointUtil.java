...
public class AuthenticationEndpointUtil {
    ...
    /**
     * Validates a given URL string to ensure it's safe for use. It checks for several common security vulnerabilities
     * and invalid states:
     *
     * Ensures the URL is not the string "null" (case-insensitive). 
     * Ensures the URL is not null, empty, or consists only of whitespace. 
     * Prevents common trickery by disallowing URLs starting with "javascript:", "file:", "ftp:", or "data:"
     *
     * @param url The URL string to be validated.
     * @return {@code true} if the URL is considered safe and valid based on the defined criteria; {@code false}
     *         otherwise.
     */
    public static boolean isSchemeSafeURL(String url) {
        ...
        return !StringUtils.equalsIgnoreCase(url, "null") && StringUtils.isNotBlank(url) &&
                !lowerCaseUrl.startsWith("javascript:") && !lowerCaseUrl.startsWith("file:") &&
                !lowerCaseUrl.startsWith("ftp:") && !lowerCaseUrl.startsWith("data:");
    }
}
