...
public class UserSessionDAOImpl implements UserSessionDAO {
    ...
    public void storeUserSessionData(String userId, String sessionId) throws UserSessionException {
        ...
        try {
            ...
            // Handle constraint violation issues, including the ones from JDBC drivers which do not throw
            // SQLIntegrityConstraintViolationException.
            if (cause instanceof SQLIntegrityConstraintViolationException || (cause instanceof SQLException
                    && StringUtils.containsIgnoreCase(cause.getMessage(), "USER_SESSION_STORE_CONSTRAINT"))) {
                throw new DuplicatedAuthUserException("Mapping between user Id: " + userId + " and session Id: "
                        + sessionId + " already exists in the database.", e);
            }
    ...
    public boolean isExistingMapping(String userId, String sessionId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate(JdbcUtils.Database.SESSION);
        try {
            return jdbcTemplate.fetchSingleRecord(SQLQueries.SQL_SELECT_USER_SESSION_MAP,
    ...
    public List<String> getSessionId(String userId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate(JdbcUtils.Database.SESSION);
        try {
            return jdbcTemplate.executeQuery(SQLQueries.SQL_SELECT_SESSION_ID_OF_USER_ID,
                    (resultSet, rowNumber) -> resultSet.getString(1),
    ...
    public List<String> getActiveSessionIds(String userId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate(JdbcUtils.Database.SESSION);
        try {
            return jdbcTemplate.executeQuery(SQLQueries.SQL_SELECT_ACTIVE_SESSION_IDS_OF_USER_ID,
                    (resultSet, rowNumber) -> resultSet.getString(1),
                    preparedStatement -> preparedStatement.setString(1, userId));
        } catch (DataAccessException e) {
    ...
    public void removeExpiredSessionRecords() {
        ...
        try {
            jdbcTemplate.withTransaction(template -> {
                Set<String> terminatedAuthSessionIds = getSessionsTerminated(template);
                String[] sessionsToRemove = terminatedAuthSessionIds.toArray(new String[0]);

                if (!terminatedAuthSessionIds.isEmpty()) {

                    if (log.isDebugEnabled()) {
                        log.debug(terminatedAuthSessionIds.size() + " number of sessions should be removed from " +
                                "the database. Removing in " + getDeleteChunkSize() + " size batches.");
                    }

                    deleteSessionDataFromTable(sessionsToRemove, template, IDN_AUTH_USER_SESSION_MAPPING_TABLE,
                            SQLQueries.SQL_DELETE_TERMINATED_SESSION_DATA);
                    deleteSessionDataFromTable(sessionsToRemove, template, IDN_AUTH_SESSION_APP_INFO_TABLE,
                            SQLQueries.SQL_DELETE_IDN_AUTH_SESSION_APP_INFO);
                    deleteSessionDataFromTable(sessionsToRemove, template, IDN_AUTH_SESSION_META_DATA_TABLE,
    ...
    public List<String> getSessionId(User user, int idpId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate(JdbcUtils.Database.SESSION);
        try {
            return jdbcTemplate.executeQuery(SQLQueries.SQL_GET_SESSIONS_BY_USER,
                    (resultSet, rowNumber) -> resultSet.getString(1),
                    preparedStatement -> {
    ...
    public boolean isExistingMapping(User user, int idpId, String sessionId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate(JdbcUtils.Database.SESSION);
        try {
            return jdbcTemplate.fetchSingleRecord(SQLQueries.SQL_GET_SESSION_MAPPING_BY_USER,
                    (resultSet, rowNumber) -> resultSet.getString(1),
                    preparedStatement -> {
                        preparedStatement.setString(1, sessionId);
    ...
    public void storeFederatedAuthSessionInfo(String sessionContextKey, AuthHistory authHistory)
        ...
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_STORE_FEDERATED_AUTH_SESSION_INFO, preparedStatement -> {
                preparedStatement.setString(1, authHistory.getIdpSessionIndex());
                preparedStatement.setString(2, sessionContextKey);
                preparedStatement.setString(3, authHistory.getIdpName());
                preparedStatement.setString(4, authHistory.getAuthenticatorName());
                preparedStatement.setString(5, authHistory.getRequestType());
            });
        } catch (DataAccessException e) {
    ...
    public void storeFederatedAuthSessionInfo(String sessionContextKey, AuthHistory authHistory, int tenantId)
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_STORE_FEDERATED_AUTH_SESSION_INFO_WITH_TENANT,
                    preparedStatement -> {
                        preparedStatement.setString(1, authHistory.getIdpSessionIndex());
                        preparedStatement.setString(2, sessionContextKey);
                        preparedStatement.setString(3, authHistory.getIdpName());
    ...
    public void storeFederatedAuthSessionInfoWithIdpId(String sessionContextKey, AuthHistory authHistory, int idpId)
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_STORE_FEDERATED_AUTH_SESSION_INFO_WITH_IDP_ID,
                    preparedStatement -> {
                        preparedStatement.setString(1, authHistory.getIdpSessionIndex());
                        preparedStatement.setString(2, sessionContextKey);
                        preparedStatement.setString(3, authHistory.getIdpName());
                        preparedStatement.setString(4, authHistory.getAuthenticatorName());
                        preparedStatement.setString(5, authHistory.getRequestType());
                        preparedStatement.setInt(6, idpId);
                    });
        } catch (DataAccessException e) {
            String msg = String.format("Error while adding session details of the session index: %s, IdP: %s " +
    ...
    public void storeFederatedAuthSessionInfo(String sessionContextKey, AuthHistory authHistory, int tenantId,
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_STORE_FEDERATED_AUTH_SESSION_INFO_WITH_TENANT_AND_IDP_ID,
                    preparedStatement -> {
                        preparedStatement.setString(1, authHistory.getIdpSessionIndex());
                        preparedStatement.setString(2, sessionContextKey);
                        preparedStatement.setString(3, authHistory.getIdpName());
                        ...
                        preparedStatement.setString(5, authHistory.getRequestType());
                        preparedStatement.setInt(6, tenantId);
                        preparedStatement.setInt(7, idpId);
                    });
        } catch (DataAccessException e) {
            String msg = String.format("Error while adding session details of the session index: %s, IdP: %s " +
                    "and tenant id: %s.", sessionContextKey, authHistory.getIdpName(), tenantId);
    ...
    public boolean hasExistingFederatedAuthSession(String idpSessionIndex) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            return jdbcTemplate.fetchSingleRecord(SQLQueries.SQL_GET_FEDERATED_AUTH_SESSION_ID_BY_SESSION_ID,
                    (resultSet, rowNumber) -> resultSet.getString(1),
                    preparedStatement -> preparedStatement.setString(1, idpSessionIndex)) != null;
        } catch (DataAccessException e) {
    ...
    public boolean isExistingFederatedAuthSessionAvailable(String idpSessionIndex, int tenantId)
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            return jdbcTemplate.fetchSingleRecord(
                    SQLQueries.SQL_GET_FEDERATED_AUTH_SESSION_ID_BY_SESSION_ID_WITH_TENANT,
                    (resultSet, rowNumber) -> resultSet.getString(1),
                    preparedStatement -> {
                        preparedStatement.setString(1, idpSessionIndex);
                        preparedStatement.setInt(2, tenantId);
                    }) != null;
        } catch (DataAccessException e) {
    ...
    public void removeFederatedAuthSessionInfo(String sessionContextKey) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_DELETE_FEDERATED_AUTH_SESSION_INFO,
                    preparedStatement -> preparedStatement.setString(1, sessionContextKey));
        } catch (DataAccessException e) {
    ...
    public void removeFederatedAuthSessionInfo(String sessionContextKey, int idpId) throws UserSessionException {
        ...
        JdbcTemplate jdbcTemplate = JdbcUtils.getNewTemplate();
        try {
            jdbcTemplate.executeUpdate(SQLQueries.SQL_DELETE_FEDERATED_AUTH_SESSION_INFO_WITH_IDP_ID,
                    preparedStatement -> {
                        preparedStatement.setString(1, sessionContextKey);
                        preparedStatement.setInt(2, idpId);
                    });
    ...
    private Set<String> getSessionsTerminated(Template<?> template) throws DataAccessException {
         ...
         * existing entry.
         */
        template.executeQuery(SQLQueries.SQL_SELECT_TERMINATED_SESSION_IDS,
                (resultSet, rowNumber) -> terminatedSessionIds.add(resultSet.getString(1)),
                preparedStatement -> preparedStatement.setLong(1, FrameworkUtils.getCurrentStandardNano()));
     ...
     * @param template         jdbc template bound to the on-going transaction
     * @param tableName        table name from which the records are removed
     * @param deleteQuery      delete query for the relevant table
     * @throws DataAccessException if the DB execution fails
     */
    private void deleteSessionDataFromTable(String[] sessionsToRemove, Template<?> template, String tableName,
                                            String deleteQuery) throws DataAccessException {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            int chunkStart = startCount;
            int chunkEnd = endCount;
            // Reuses the generic batch executor (add-batch-then-executeBatch) for the chunked delete.
            template.executeBatchInsert(deleteQuery, preparedStatement -> {
                for (int j = chunkStart; j < chunkEnd; j++) {
                    preparedStatement.setString(1, sessionsToRemove[j]);
                    preparedStatement.addBatch();
                }
            }, null);
