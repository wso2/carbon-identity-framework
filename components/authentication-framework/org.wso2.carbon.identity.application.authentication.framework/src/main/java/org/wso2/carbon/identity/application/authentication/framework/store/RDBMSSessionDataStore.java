...
public class RDBMSSessionDataStore extends SessionDataStore {
    ...
    private static final String SQL_INSERT =
            "INSERT INTO IDN_AUTH_SESSION_STORE(SESSION_ID, SESSION_TYPE, SESSION_OBJECT,TIME_CREATED, EXPIRY_TIME, TENANT_ID) " +
                    "VALUES " + "(?,?,?,?,?,?)";
    private static final String SQL_DELETE =
            "DELETE FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID = ? AND  SESSION_TYPE = ?";
    ...
    private static final String SQL_DESERIALIZE_OBJECT_MYSQL =
            "SELECT SESSION_OBJECT,TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
                    " SESSION_TYPE=? ";
    private static final String SQL_DESERIALIZE_OBJECT_DB2SQL =
            "SELECT SESSION_OBJECT,TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
                    " SESSION_TYPE=? ";
    private static final String SQL_DESERIALIZE_OBJECT_MSSQL =
            "SELECT TOP 1 SESSION_OBJECT,TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
                    " SESSION_TYPE=? ";
    private static final String SQL_DESERIALIZE_OBJECT_POSTGRESQL =
            "SELECT SESSION_OBJECT,TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
                    " SESSION_TYPE=? ";
    private static final String SQL_DESERIALIZE_OBJECT_INFORMIX =
            "SELECT FIRST 1 SESSION_OBJECT, TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
                    " SESSION_TYPE=? ";
    private static final String SQL_DESERIALIZE_OBJECT_ORACLE =
            "SELECT * FROM (SELECT SESSION_OBJECT, TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE SESSION_ID =? AND" +
    ...
    private static final String SQL_DELETE_EXPIRED_DATA_TASK_INFOMIXSQL = "DELETE FROM (SELECT SESSION_ID, " +
            "SESSION_TYPE,TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE EXPIRY_TIME < ? LIMIT %d) ";
    private static final String SQL_DELETE_EXPIRED_DATA_TASK_DB2SQL =
            "DELETE FROM IDN_AUTH_SESSION_STORE WHERE (SESSION_ID, SESSION_TYPE, TIME_CREATED) IN " +
                    "(SELECT SESSION_ID, SESSION_TYPE, TIME_CREATED FROM IDN_AUTH_SESSION_STORE WHERE " +
                    "EXPIRY_TIME < ? FETCH FIRST %d ROWS ONLY)";
    private static final String SQL_UPDATE = "UPDATE IDN_AUTH_SESSION_STORE SET SESSION_OBJECT = ?,TIME_CREATED = ?, " +
...

    {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Session data pool size config value: " + maxSessionDataPoolSize);
            }
            tempDataCleanupEnabled = getBooleanProperty(FrameworkConstants.SessionDataStoreConstants.GET_TEMP_CLEANUP_ENABLE,
                    FrameworkConstants.SessionDataStoreConstants.DEFAULT_TEMP_DATA_CLEANUP_ENABLED);
            maxTempDataPoolSize = getIntProperty(FrameworkConstants.SessionDataStoreConstants.GET_TEMP_POOL_SIZE,
                    FrameworkConstants.SessionDataStoreConstants.DEFAULT_MAX_TEMP_DATA_POOLSIZE);
            if (log.isDebugEnabled()) {
                log.debug("Temporary data pool size config value: " + maxTempDataPoolSize);
            }
    ...
    RDBMSSessionDataStore() {
        ...
        deleteChunkSize = getIntProperty(FrameworkConstants.SessionDataStoreConstants.GET_DELETE_CHUNK_SIZE,
