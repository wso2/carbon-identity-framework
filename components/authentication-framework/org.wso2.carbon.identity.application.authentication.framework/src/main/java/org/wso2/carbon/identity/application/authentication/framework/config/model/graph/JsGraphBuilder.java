                ...
                authenticatorConfig.getIdps()
                        .forEach((idpName, idp) -> {
                            ...
                            if (authenticators == null) {
                                ...
                                // Both idp and authenticator present, but authenticator is given by display name due
                                // to the fact
                                // that it is the one available at UI. Should translate the display name to actual name,
                                // and keep/remove option
                                removeOption = true;
                                ...
                                if (FrameworkConstants.LOCAL_IDP_NAME.equals(idpName)) {
                                    ...
                                    if (log.isDebugEnabled()) {
                                        if (removeOption) {
                                            log.debug(String.format("Authenticator options don't match any entry for " +
                                                            "local authenticator: %s. Hence removing the option",
                                                    authenticatorConfig.getName()));
                                        } else {
                                            log.debug(String.format("Authenticator options contained a match for " +
                                                            "local authenticator: %s. Hence keeping the option",
                                                    authenticatorConfig.getName()));
                                        }
                                    }
                                } else {
                                    ...
                                    if (log.isDebugEnabled()) {
                                        if (removeOption) {
                                            log.debug(String.format("Authenticator options don't match any entry for " +
                                                            "idp: %s, authenticator: %s. Hence removing the option",
                                                    idpName, authenticatorConfig.getName()));
                                        } else {
                                            log.debug(String.format("Authenticator options contained a match for " +
                                                            "idp: %s, authenticator: %s. Hence keeping the option",
                                                    idpName, authenticatorConfig.getName()));
                                        }
                                    }
                                }
                                ...
                                if (log.isDebugEnabled()) {
                                    log.debug(String.format("No authenticator filters for idp %s, hence keeping it " +
                                                    "as an option", idpName));
                                }
                            }
                            if (removeOption) {
                                try {
