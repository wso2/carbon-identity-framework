...
public class SessionMgtUtils {
    ...
    public static String getConfiguredSessionStoreName() {
        ...
        String storeName = IdentityUtil.getProperty(SESSION_STORAGE_TYPE_PROPERTY);
        return StringUtils.isBlank(storeName) ? DEFAULT_SESSION_STORE_NAME
                : storeName.trim().toLowerCase(Locale.ENGLISH);
    }

    /**
     * Returns the properties configured for the session storage, which are the settings of the store
     * named by {@link #getConfiguredSessionStoreName()}. A store defines its own property names, and
     * reads the ones it knows from here; no name is interpreted by the framework.
     *
    ...
    public static Map<String, String> getSessionStorageProperties() {
        ...
        return IdentityConfigParser.getSessionStorageProperties();
    }
