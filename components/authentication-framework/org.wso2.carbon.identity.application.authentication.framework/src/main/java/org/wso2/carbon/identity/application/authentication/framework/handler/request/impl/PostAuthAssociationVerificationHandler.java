...
public class PostAuthAssociationVerificationHandler extends AbstractPostAuthnHandler {
    ...
    public int getPriority() {
        ...
        if (priority == -1) {
            // This should execute before JIT provisioning handler.
            priority = 35;
        }
        return priority;
    }
    ...
    public PostAuthnHandlerFlowStatus handle(HttpServletRequest request, HttpServletResponse response,
        ...
        if (context.getExternalIdP() == null || (context.getExternalIdP() != null && ORGANIZATION_LOGIN_IDP_NAME.equals(
                context.getExternalIdP().getIdentityProvider().getIdentityProviderName()))) {
        ...
        try {
            ...
            if (!FrameworkUtils.isLoginFailureWithNoLocalAssociationEnabledForApp(serviceProvider)) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Skipping PostAuthAssociationVerificationHandler as " +
                            "login failure with no local association is not enabled for the application: " +
            ...
            if (userLinkStrategy == UserLinkStrategy.MANDATORY) {
                User localUser = FrameworkServiceDataHolder.getInstance().getFederatedAssociationManager()
                        .getAssociatedLocalUser(tenantDomain,
