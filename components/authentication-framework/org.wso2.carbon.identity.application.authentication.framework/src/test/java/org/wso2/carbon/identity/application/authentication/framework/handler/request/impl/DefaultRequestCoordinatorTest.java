...
public class DefaultRequestCoordinatorTest extends IdentityBaseTest {
    ...
    public void testAddServiceProviderIdToRedirectUrl() throws Exception {
        ...
        // Case 5: Service provider ID is present and appended to URL.
        responseWrapper = mock(CommonAuthResponseWrapper.class);
        context = mock(AuthenticationContext.class);
        ...
        try (MockedStatic<FrameworkUtils> frameworkUtils = mockStatic(FrameworkUtils.class)) {
            String expectedUrl = "https://example.com/redirect?param=value&spId=sp-resource-id-123";
            frameworkUtils.when(() -> FrameworkUtils.appendQueryParamsStringToUrl(
                    anyString(), anyString()
        ...
        responseWrapper = mock(CommonAuthResponseWrapper.class);
        context = mock(AuthenticationContext.class);
        // Use a URL that contains a query string to avoid a null rawQuery in URI parsing inside the method.
        ...
        try (MockedStatic<FrameworkUtils> frameworkUtils = mockStatic(FrameworkUtils.class)) {
            String expectedUrl = "https://example.com/redirect?spId=sp-resource-id-456";
            frameworkUtils.when(() -> FrameworkUtils.appendQueryParamsStringToUrl(
                    anyString(), anyString()
            )).thenReturn(expectedUrl);
