...
/**
 * Unit tests for {@link ConsentAppMappingApplicationDeleteListener}.
 */
public class ConsentAppMappingApplicationDeleteListenerTest {

    private static final String APP_RESOURCE_ID = "app-resource-id-1";
    ...
    private ConsentAppMappingApplicationDeleteListener listener;
    ...
    public void setUp() {
        ...
        openMocks(this);
        listener = new ConsentAppMappingApplicationDeleteListener();
    ...
    public void testGetDefaultOrderId() {
        ...
        assertEquals(listener.getDefaultOrderId(), 908);
    }

    @Test
    public void testDoPostDeleteApplication_success() throws Exception {

        ServiceProvider serviceProvider = mock(ServiceProvider.class);
        when(serviceProvider.getApplicationResourceId()).thenReturn(APP_RESOURCE_ID);

        boolean result = listener.doPostDeleteApplication(serviceProvider, TENANT_DOMAIN, USER_NAME);

        assertTrue(result);
        verify(consentAppMappingService).removeAllPurposeMappingsForApplication(APP_RESOURCE_ID);
    }

    @Test
    public void testDoPostDeleteApplication_exceptionIsWarned() throws Exception {

        ServiceProvider serviceProvider = mock(ServiceProvider.class);
        when(serviceProvider.getApplicationResourceId()).thenReturn(APP_RESOURCE_ID);
        doThrow(new ConsentAppMappingException("Test error"))
                .when(consentAppMappingService).removeAllPurposeMappingsForApplication(APP_RESOURCE_ID);
