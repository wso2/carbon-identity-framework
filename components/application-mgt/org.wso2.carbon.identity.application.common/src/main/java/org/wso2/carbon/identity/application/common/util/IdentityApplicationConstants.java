...
public class IdentityApplicationConstants {
    ...
    public static final String REF =
            "/**\n" +
                    ...
                    "\n" +
                    "// Error page to redirect unauthorized users after the account lock\n" +
                    "var errorPage = '';\n" +
                    "\n" +
                    "var errorPageParameters = {\n" +
                    "    'status': 'Suspicious Login Attempt',\n" +
                    "    'statusMsg': 'Suspicious login attaempt was detected. Your account was locked. Please contact administrator'\n" +
                    "};\n" +
                    "\n" +
                    "         \n" +
                    ...
                    "            var failedLoginAttempts = user.localClaims[failedLoginAttemptsBeforeSuccessClaim];\n" +
                    "            subject = context.currentKnownSubject;\n" +
                    "            if (failedLoginAttempts > 0) {\n" +
                    "                riskScore =  parseInt(riskScore) +  parseInt(failedLoginAttempts);\n" +
                    "                Log.info('--------------- Risk score based on FAILED LOGIN ATTEMPTS: ' + riskScore);\n" +
                    "            }\n" +
                    "            if (!validateCookie(context, subject)) {\n" +
                    "                riskScore = parseInt(riskScore)+2;\n" +
                    "                setCookie(context.response, cookieName, subject.identifier, {\n" +
                    "                    'sign': true,\n" +
                    "                    'max-age': deviceRememberPeriod\n" +
                    "                });   \n" +
                    "            \tLog.info('--------------- Risk score based on NEW DEVICE: ' + riskScore);\n" +
                    "            }\n" +
                    "            callAnalytics({'Application':'IS_ANALYTICS_RISK_BASED_LOGIN','InputStream':'InputStream'}, {'username':username, 'userTenantDomain':userTenantDomain, 'riskScore':riskScore}, {\n" +
                    "                onSuccess : function(context, data) {\n" +
                    "                    var user = context.steps[1].subject;\n" +
                    "                    Log.info('--------------- Received risk score value from analytics: ' + data.event.riskScore);\n" +
                    "                    if (data.event.riskScore > 5) {\n" +
                    "                        user.localClaims['http://wso2.org/claims/identity/accountLocked'] = true;\n" +
                    "                        sendError(errorPage, errorPageParameters);\n" +
                    "                    } else if (data.event.riskScore > 3) {\n" +
                    "                        executeStep(3);\n" +
                    "                    } else if (data.event.riskScore > 1) {\n" +
                    "                        executeStep(2);\n" +
                    "                    }\n" +
