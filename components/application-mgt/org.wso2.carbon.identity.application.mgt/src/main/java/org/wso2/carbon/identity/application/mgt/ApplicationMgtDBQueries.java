...
public class ApplicationMgtDBQueries {
    ...
    public static final String LOAD_APP_NAMES_BY_TENANT_AND_APP_NAME = "SELECT APP_NAME, DESCRIPTION FROM SP_APP WHERE " +
            "TENANT_ID = ? AND APP_NAME LIKE ? ORDER BY ID DESC";
    ...
    public static final String LOAD_APP_NAMES_BY_TENANT_AND_APP_NAME_MYSQL = "SELECT ID, APP_NAME, DESCRIPTION , UUID, " +
            "IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID FROM SP_APP WHERE TENANT_ID = ? " +
            "AND APP_NAME LIKE ? ORDER BY ID DESC LIMIT ?, ?";

    public static final String LOAD_APP_NAMES_BY_TENANT_ORACLE = "SELECT ID, APP_NAME, DESCRIPTION, UUID, IMAGE_URL, " +
            "LOGIN_URL, USERNAME, USER_STORE, TENANT_ID FROM " +
            "(SELECT ID, APP_NAME, DESCRIPTION, DESCRIPTION, UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, " +
            "TENANT_ID, rownum AS rnum FROM " +
            "(SELECT ID, APP_NAME, DESCRIPTION, UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID " +
            "FROM SP_APP ORDER BY ID DESC) WHERE TENANT_ID = ? AND rownum <= ?) WHERE rnum > ?";
    public static final String LOAD_APP_NAMES_BY_TENANT_AND_APP_NAME_ORACLE = "SELECT ID, APP_NAME, DESCRIPTION, " +
            "UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID FROM " +
            "(SELECT ID, APP_NAME, DESCRIPTION, UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID, rownum AS rnum FROM " +
            "(SELECT ID, APP_NAME, DESCRIPTION, UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, " +
            "TENANT_ID FROM SP_APP ORDER BY ID DESC) WHERE TENANT_ID = ? AND APP_NAME LIKE ? AND rownum <= ?) WHERE rnum > ?";

    public static final String LOAD_APP_NAMES_BY_TENANT_DB2SQL = "SELECT ID, APP_NAME, DESCRIPTION, " +
            "UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID FROM (SELECT "
    ...
    public static final String LOAD_APP_NAMES_BY_TENANT_INFORMIX = "SELECT SKIP ? FIRST ? ID, APP_NAME, " +
            "DESCRIPTION, UUID, IMAGE_URL, LOGIN_URL, USERNAME, USER_STORE, TENANT_ID FROM SP_APP " +
