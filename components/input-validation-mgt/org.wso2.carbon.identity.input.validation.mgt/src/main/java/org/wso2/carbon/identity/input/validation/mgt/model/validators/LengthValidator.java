...
public class LengthValidator extends AbstractRulesValidator {
    ...
    public boolean validate(ValidationContext context) throws InputValidationMgtClientException {
        ...
        if (attributesMap.containsKey(field + "." + MIN_LENGTH)) {
            ...
            if (length < min) {
                throw new InputValidationMgtClientException(ERROR_VALIDATION_MIN_LENGTH_MISMATCH.getCode(),
                        ERROR_VALIDATION_MIN_LENGTH_MISMATCH.getMessage(),
                        String.format(ERROR_VALIDATION_MIN_LENGTH_MISMATCH.getDescription(), field, min));
            }
        }
        if (attributesMap.containsKey(field + "." + MAX_LENGTH)) {
            ...
            if (length > max) {
                throw new InputValidationMgtClientException(ERROR_VALIDATION_MAX_LENGTH_MISMATCH.getCode(),
                        ERROR_VALIDATION_MAX_LENGTH_MISMATCH.getMessage(),
                        String.format(ERROR_VALIDATION_MAX_LENGTH_MISMATCH.getDescription(), field, max));
            }
        }
        return true;
    }
