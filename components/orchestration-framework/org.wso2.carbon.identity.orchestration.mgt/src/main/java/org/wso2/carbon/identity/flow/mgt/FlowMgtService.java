...
public class FlowMgtService {
    ...
    public void updateFlow(FlowDTO flowDTO, int tenantID)
        ...
        flowDAO.updateFlow(flowConfig, tenantID, Constants.DEFAULT_FLOW_NAME, flowDTO.getFlowType(),
                flowDTO.getFlowCategory(),flowDTO.isDefault());
     ...
     * @return The flow.
     * @throws OrchestrationFrameworkException If an error occurs while retrieving the flow.
     */
    public FlowDTO getFlow(int tenantID, String flowType) throws OrchestrationFrameworkException {

        return flowDAO.getFlow(tenantID, flowType);
    }

    /**
     * Get the specified flow by tenant ID.
     *
     * @param tenantID The tenant ID.
     * @param flowType The type of the flow.
     * @return The built {@link GraphConfig}.
     * @throws OrchestrationFrameworkException If an error occurs while retrieving the graph.
     */
    public GraphConfig getGraphConfig(int tenantID, String flowType) throws OrchestrationFrameworkException {

        return flowDAO.getGraph(tenantID, flowType);
    }
}
