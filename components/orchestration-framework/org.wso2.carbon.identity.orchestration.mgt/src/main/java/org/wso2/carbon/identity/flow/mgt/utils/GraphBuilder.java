...
import static org.wso2.carbon.identity.flow.mgt.utils.OrchestrationMgtUtils.handleClientException;
...
public class GraphBuilder {
     ...
     * @throws OrchestrationFrameworkException If an error occurs while processing the steps.
     */
    public GraphBuilder withSteps(List<StepDTO> steps) throws OrchestrationFrameworkException {
        ...
        for (StepDTO step : steps) {
            switch (step.getType()) {
                ...
                case Constants.StepTypes.REDIRECTION:
                    ...
                    break;
                case Constants.StepTypes.USER_ONBOARD:
     ...
     * @throws OrchestrationFrameworkException If an error occurs during graph configuration.
     */
    public GraphConfig build() throws OrchestrationFrameworkException {
    ...
    private void processRedirectionStep(StepDTO step) throws OrchestrationFrameworkException {
        ...
        if (action == null) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_ACTION_DATA_NOT_FOUND, step.getId());
        }
        if (!Constants.ActionTypes.EXECUTOR.equals(action.getType())) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_INVALID_ACTION_TYPE, action.getType(), step.getId());
        }
        if (action.getExecutor() == null) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_EXECUTOR_INFO_NOT_FOUND, step.getId());
        }
    ...
    private void processViewStep(StepDTO step)
        ...
        if (components == null || components.isEmpty()) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_COMPONENT_DATA_NOT_FOUND, step.getId());
        }
    ...
    private void processComponent(ComponentDTO component, List<NodeConfig> stepNodes, String stepId)
        ...
        } else if (Constants.ComponentTypes.BUTTON.equals(component.getType())) {
    ...
    private void validateStepActions(ActionDTO action, List<NodeConfig> stepNodes, String id, String stepId)
        ...
        if (action == null) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_INVALID_ACTION_FOR_BUTTON, id);
        }
        if (action.getNextId() == null) {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_NEXT_ACTION_NOT_FOUND, id);
        }
        if (Constants.ActionTypes.EXECUTOR.equals(action.getType())) {
            if (action.getExecutor() == null) {
                throw handleClientException(Constants.ErrorMessages.ERROR_CODE_EXECUTOR_INFO_NOT_FOUND, id);
            }
            if (stepNodes.stream().anyMatch(nodeConfig -> (Constants.NodeTypes.TASK_EXECUTION.equals(nodeConfig.getType())))) {
                throw handleClientException(Constants.ErrorMessages.ERROR_CODE_MULTIPLE_STEP_EXECUTORS, stepId);
            }
        }
    }
    ...
    private NodeConfig createNodeFromAction(ActionDTO action, String componentId)
        ...
        if (Constants.ActionTypes.NEXT.equals(action.getType())) {
            tempNodeInComponent = createPagePromptNode(componentId);
        } else if (Constants.ActionTypes.EXECUTOR.equals(action.getType())) {
            tempNodeInComponent = createTaskExecutionNode(componentId, action.getExecutor());
        } else {
            throw handleClientException(Constants.ErrorMessages.ERROR_CODE_UNSUPPORTED_ACTION_TYPE, action.getType(), componentId);
        }
    ...
    private void handleTempNodesInStep(List<NodeConfig> tempNodesInStep, StepDTO step) {
        ...
        if (tempNodesInStep.size() > 1) {
            ...
            for (NodeConfig nodeConfig : tempNodesInStep) {
                if (Constants.NodeTypes.TASK_EXECUTION.equals(nodeConfig.getType())) {
                    if (LOG.isDebugEnabled()) {
    ...
    private void resolveGraphEdgesAndFirstNode() throws OrchestrationFrameworkException {
        ...
        if (firstNodeIds.size() != 1) {
            throw handleServerException(Constants.ErrorMessages.ERROR_CODE_INVALID_FIRST_NODE);
        }
    ...
    private NodeConfig createDecisionNode(String id) {
        ...
        NodeConfig nodeConfig = new NodeConfig.Builder()
                .id(id)
                .type(Constants.NodeTypes.DECISION)
                .build();
        if (LOG.isDebugEnabled()) {
    ...
    private NodeConfig createUserOnboardingNode(String stepId) {
        ...
        NodeConfig nodeConfig = new NodeConfig.Builder()
                .id(stepId)
                .type(Constants.NodeTypes.TASK_EXECUTION)
                .executorConfig(new ExecutorDTO(Constants.ExecutorTypes.USER_ONBOARDING))
                .build();
        if (LOG.isDebugEnabled()) {
