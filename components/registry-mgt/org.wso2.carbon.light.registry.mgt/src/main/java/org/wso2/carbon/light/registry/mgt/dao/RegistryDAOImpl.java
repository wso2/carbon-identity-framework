...
public class RegistryDAOImpl implements RegistryDAO {
    ...
    private static final RegistryDAO instance = new RegistryDAOImpl();

    private RegistryDAOImpl() {
    ...
    @Override
    public boolean resourceExists(ResourceID resourceId) throws LightRegistryException {

        boolean isExists;
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            isExists =
                    executeResourceExists(connection, resourceId.getPathID(), resourceId.getName(),
    ...
    public int getPathID(String path, int tenantId) throws LightRegistryException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.GET_PATH_ID)) {
    ...
    @Override
    public ResourceImpl getResourceMetaData(ResourceID resourceID) throws LightRegistryException {

        ResourceImpl resource;
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            resource = executeGetResource(connection, resourceID);
        } catch (SQLException e) {
    ...
    @Override
    public void fillResource(ResourceImpl resourceImpl) throws LightRegistryException {
        ...
        if (!(resourceImpl instanceof CollectionImpl) && resourceImpl.getDbBasedContentID() > 0) {
            ...
            if (resourceImpl instanceof CollectionImpl) {
                fillChildren((CollectionImpl) resourceImpl);
            }
        }
        fillResourceProperties(resourceImpl.getResourceID(), resourceImpl);
    }

    @Override
    public void fillResourceProperties(ResourceID resourceId, ResourceImpl resource)
            throws LightRegistryException {

        Map<String, String> properties;
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            properties = executeGetResourceProperties(connection, resourceId);
        } catch (SQLException e) {
    ...
    @Override
    public void add(String path, int tenantId, ResourceID parentId, ResourceImpl resourceImpl)
            throws LightRegistryException {

        // creating the resourceID
        createAndApplyResourceID(path, tenantId, parentId, resourceImpl);

        addResourceWithoutContentId(resourceImpl);
    ...
    public void deleteContent(int contentId, int tenantId) throws LightRegistryException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.DELETE_CONTENT)) {
    ...
    @Override
    public void update(ResourceImpl resourceImpl) throws LightRegistryException {
    ...
    @Override
    public void fillChildren(CollectionImpl collection) throws LightRegistryException {

        String[] childPaths = getChildren(collection);
    ...
    @Override
    public String[] getChildren(CollectionImpl collection) throws LightRegistryException {

        List<String> children = new ArrayList<>();
        String path = collection.getPath();
        int pathId = collection.getPathID();
        int tenantId = collection.getTenantId();
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.GET_COLLECTION_CHILD_RESOURCES)) {
                statement.setInt(REG_PATH_ID, pathId);
                statement.setInt(TENANT_ID, tenantId);
                try (ResultSet resultSet = statement.executeQuery()) {
            ...
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.GET_COLLECTION_CHILD_PATHS)) {
    ...
    @Override
    public void fillResourceContent(ResourceImpl resourceImpl) throws LightRegistryException {

        int contentId = resourceImpl.getDbBasedContentID();
        int tenantId = resourceImpl.getTenantId();
        byte[] content = null;
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
    ...
    @Override
    public void updateContentId(ResourceImpl resourceImpl) throws LightRegistryException {

        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.UPDATE_RESOURCE)) {
                statement.setInt(REG_CONTENT_ID, resourceImpl.getDbBasedContentID());
                statement.setInt(REG_PATH_ID, resourceImpl.getPathID());
                statement.setInt(TENANT_ID, resourceImpl.getTenantId());
    ...
    public void addResourceWithoutContentId(ResourceImpl resourceImpl)
        ...
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.ADD_RESOURCE)) {
                statement.setInt(REG_PATH_ID, resourceImpl.getPathID());
                statement.setString(REG_NAME, resourceImpl.getName());
    ...
    @Override
    public void deleteResource(ResourceImpl resource) throws LightRegistryException {

        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            executeDeleteResource(connection, resource);
        } catch (SQLException e) {
    ...
    @Override
    public void removeProperties(ResourceImpl resource) throws LightRegistryException {

        Integer[] propertyIds = getPropertyIds(resource);
        int tenantId = resource.getTenantId();

        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement1 = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.DELETE_RESOURCE_PROPERTY)) {
                NamedPreparedStatement statement2 = new NamedPreparedStatement(connection,
                        RegistrySQLQueries.DELETE_PROPERTY);
    ...
    @Override
    public void addContent(ResourceImpl resourceImpl) throws LightRegistryException {

        if (!(resourceImpl.getContent() instanceof byte[])) {
            resourceImpl.setDbBasedContentID(0);
            return;
        }
        int contentID = addContentBytes(resourceImpl.getContent(), resourceImpl.getTenantId());
    ...
    public int addContentBytes(Object content, int tenantId) throws LightRegistryException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                    RegistrySQLQueries.ADD_CONTENT,
                    DBUtils.getConvertedAutoGeneratedColumnName(connection.getMetaData().getDatabaseProductName(),
                            REG_CONTENT_ID))) {
            ...
            try (NamedPreparedStatement statement = new NamedPreparedStatement(connection,
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        ...
                        childResourceId.setTenantId(tenantId);
    ...
    private void addProperty(Connection connection, String key, String value, ResourceID resourceId)
        ...
        try (NamedPreparedStatement statement1 = new NamedPreparedStatement(connection,
                RegistrySQLQueries.ADD_PROPERTY,
                DBUtils.getConvertedAutoGeneratedColumnName(connection.getMetaData().getDatabaseProductName(),
                        REG_ID))) {
            ...
            try (ResultSet resultSet = statement1.getGeneratedKeys()) {
                if (resultSet.next()) {
                    ...
                    NamedPreparedStatement statement2 =
                            new NamedPreparedStatement(connection, RegistrySQLQueries.ADD_RESOURCE_PROPERTY);
                    statement2.setInt(REG_PROPERTY_ID, propertyId);
                    statement2.setInt(REG_PATH_ID, pathId);
    ...
    private Integer[] getPropertyIds(ResourceImpl resource) throws LightRegistryException {
        ...
        try (Connection connection = IdentityDatabaseUtil.getGovernanceDBConnection(false)) {
            ...
            if (resource instanceof CollectionImpl) {
                statement1 = new NamedPreparedStatement(connection, RegistrySQLQueries.GET_COLLECTION_RESOURCE_PROPERTY_ID);
                statement1.setInt(REG_PATH_ID, resource.getPathID());
                statement1.setInt(TENANT_ID, resource.getTenantId());
            } else {
                statement1 = new NamedPreparedStatement(connection, RegistrySQLQueries.GET_FILE_RESOURCE_PROPERTY_ID);
                statement1.setInt(REG_PATH_ID, resource.getPathID());
    ...
    private boolean executeResourceExists(Connection connection, int pathId, String resourceName, int tenantId)
        ...
        try {
            ...
            if (resourceName == null) {
                statement = new NamedPreparedStatement(connection, RegistrySQLQueries.IS_COLLECTION_RESOURCE_EXISTS);
            } else {
                statement = new NamedPreparedStatement(connection, RegistrySQLQueries.IS_FILE_RESOURCE_EXISTS);
    ...
    private ResourceImpl executeGetResource(Connection connection, ResourceID resourceID)
            throws LightRegistryException {
    ...
    private Map<String, String> executeGetResourceProperties(Connection connection, ResourceID resourceId)
            throws LightRegistryException {
        ...
        try {
            ...
            if (isCollection) {
                statement = new NamedPreparedStatement(connection,
                        RegistrySQLQueries.GET_COLLECTION_RESOURCE_PROPERTIES);
            } else {
                statement = new NamedPreparedStatement(connection,
                        RegistrySQLQueries.GET_FILE_RESOURCE_PROPERTIES);
                statement.setString(REG_NAME, resourceId.getName());
            }
            statement.setInt(REG_PATH_ID, resourceId.getPathID());
            statement.setInt(TENANT_ID, tenantId);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
    ...
    private void executeDeleteResource(Connection connection, ResourceImpl resource) throws LightRegistryException {

        try {
            NamedPreparedStatement statement;
            if (resource instanceof CollectionImpl) {
                statement =
                        new NamedPreparedStatement(connection, RegistrySQLQueries.DELETE_COLLECTION_RESOURCE);
            } else {
                statement = new NamedPreparedStatement(connection, RegistrySQLQueries.DELETE_FILE_RESOURCE);
                statement.setString(REG_NAME, resource.getName());
            }
            statement.setInt(REG_PATH_ID, resource.getPathID());
            statement.setInt(TENANT_ID, resource.getTenantId());
            statement.executeUpdate();
        } catch (SQLException e) {
            throw new LightRegistryException("Error while deleting registry resource.", e);
        }
