...
public class SQLConstants {
    ...
    public static final String INSERT_OR_UPDATE_RESOURCE_ORACLE = "MERGE INTO IDN_CONFIG_RESOURCE T USING (SELECT ? " +
            "AS ID, ? AS TENANT_ID, ? AS NAME, ? AS CREATED_TIME, ? AS LAST_MODIFIED, ? AS HAS_FILE, ? AS " +
            "HAS_ATTRIBUTE, ? AS TYPE_ID FROM dual) S ON (T.ID = S.ID) WHEN MATCHED THEN UPDATE SET " +
            "T.TENANT_ID = S.TENANT_ID, T.NAME = S.NAME, T.LAST_MODIFIED = S.LAST_MODIFIED, T.HAS_FILE = S.HAS_FILE, " +
            "T.HAS_ATTRIBUTE = S.HAS_ATTRIBUTE, T.TYPE_ID = S.TYPE_ID WHEN NOT MATCHED THEN INSERT (ID, TENANT_ID, " +
            "NAME, CREATED_TIME, LAST_MODIFIED, HAS_FILE, HAS_ATTRIBUTE, TYPE_ID) VALUES (S.ID, S.TENANT_ID, S.NAME," +
            " S.CREATED_TIME, S.LAST_MODIFIED, S.HAS_FILE, S.HAS_ATTRIBUTE, S.TYPE_ID)";
