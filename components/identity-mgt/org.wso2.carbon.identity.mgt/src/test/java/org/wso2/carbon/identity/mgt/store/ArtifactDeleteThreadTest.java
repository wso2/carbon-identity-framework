...
/**
 * Unit tests for the old confirmation code cleanup in {@link ArtifactDeleteThread}.
 *
 * <p>The thread identifies the username segment of each stored resource the same way
 * {@link RegistryRecoveryDataStore} does, so usernames that carry the delimiter have to be matched correctly here as
 * well.</p>
 */
public class ArtifactDeleteThreadTest {
    ...
    /**
     * Wires a mocked registry and switches username hashing off, so the thread matches on the plain username.
     */
    @BeforeMethod
    ...
    public void testOwnCodeIsDeleted(String username) throws Exception {
        ...
        Resource own = resourceAt(ownResource);
        Resource other = resourceAt(otherResource);
        when(collection.getChildren()).thenReturn(new String[]{ownResource, otherResource});
        when(registry.get(ownResource)).thenReturn(own);
        when(registry.get(otherResource)).thenReturn(other);

        new ArtifactDeleteThread(registry, username, CODE_PATH, TENANT_ID, true).run();
    ...
    public void testSecondaryUserStoreCollectionIsTraversed() throws Exception {
        ...
        // The second read returns nothing, which ends the traversal.
        when(collection.getChildren()).thenReturn(new String[]{secondaryCollection}, new String[0]);

        new ArtifactDeleteThread(registry, "secondary/someuser", CODE_PATH, TENANT_ID, true).run();

        verify(registry, never()).delete(anyString());
    }

    private Resource resourceAt(String path) {
