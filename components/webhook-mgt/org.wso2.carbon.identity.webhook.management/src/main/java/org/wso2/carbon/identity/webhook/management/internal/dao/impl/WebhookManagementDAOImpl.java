...
public class WebhookManagementDAOImpl implements WebhookManagementDAO {
        ...
        try {
            jdbcTemplate.withTransaction(template -> {
                ...
                if (webhook.getEventsSubscribed() != null && !webhook.getEventsSubscribed().isEmpty() &&
                    ...
                    addWebhookEventsInternal(webhookInternalId, webhook.getEventsSubscribed());
                }
                return null;
            });
    ...
    public void updateWebhook(String webhookId, Webhook webhook, int tenantId) throws WebhookMgtException {
        ...
        try {
            jdbcTemplate.withTransaction(template -> {
                ...
                // Update webhook events if available
    ...
    public Webhook getWebhook(String webhookId, int tenantId) throws WebhookMgtException {
        ...
        try {
            jdbcTemplate.withTransaction(
                    template -> template.fetchSingleRecord(Query.GET_WEBHOOK_BY_ID,
                            (resultSet, rowNumber) -> {
                                ...
                                webhookData.setVersion(mappedWebhook.getVersion());
                                ...
                                webhookData.setEventSchemaVersion(mappedWebhook.getEventSchemaVersion());
            ...
            if (webhookInternalId != null) {
                webhookData.setEventsSubscribed(listWebhookEventsInternal(webhookInternalId));
            }
            return webhookData;
    ...
    /**
     * List webhook subscriptions from the database based on search filter.
     *
     * @param status   Status filter.
     * @param limit    Maximum number of results to return.
     * @param offset   Starting index of the results.
     * @param tenantId Tenant ID.
     * @return List of webhook subscriptions matching the filter.
     * @throws WebhookMgtException If an error occurs while retrieving webhook subscriptions.
     */
    @Override
    public WebhookSearchResult listWebhooks(String status, int limit, int offset, int tenantId)
        ...
        try {
            if (StringUtils.isNotEmpty(status)) {
                webhooks = jdbcTemplate.executeQuery(Query.LIST_WEBHOOKS_BY_STATUS,
                        (resultSet, rowNumber) -> mapWebhook(resultSet),
                        preparedStatement -> {
                            preparedStatement.setInt(Column.TENANT_ID, tenantId);
                            preparedStatement.setString(Column.STATUS, status);
                            preparedStatement.setInt(Column.LIMIT, limit);
                            preparedStatement.setInt(Column.OFFSET, offset);
                        });

                totalCount = jdbcTemplate.fetchSingleRecord(Query.COUNT_WEBHOOKS_BY_STATUS,
                ...
                webhooks = jdbcTemplate.executeQuery(Query.LIST_WEBHOOKS,
                        (resultSet, rowNumber) -> mapWebhook(resultSet),
                        preparedStatement -> {
                            preparedStatement.setInt(Column.TENANT_ID, tenantId);
                            preparedStatement.setInt(Column.LIMIT, limit);
                ...
                totalCount = jdbcTemplate.fetchSingleRecord(Query.COUNT_WEBHOOKS,
                        (resultSet, rowNumber) -> resultSet.getInt(1),
                        preparedStatement -> {
                            preparedStatement.setInt(Column.TENANT_ID, tenantId);
                        });
            ...
            for (Webhook webhook : webhooks) {
                ...
                if (webhookInternalId != null) {
                    webhook.setEventsSubscribed(listWebhookEventsInternal(webhookInternalId));
                }
            }

            return new WebhookSearchResult(webhooks, totalCount);
    ...
    /**
     * List webhook endpoints for a particular tenant.
     *
     * @param tenantId Tenant ID.
     * @return List of webhook endpoints.
     * @throws WebhookMgtException If an error occurs while retrieving webhook endpoints.
     */
    @Override
    public List<String> listWebhookEndpoints(int tenantId) throws WebhookMgtException {

        NamedJdbcTemplate jdbcTemplate = new NamedJdbcTemplate(IdentityDatabaseUtil.getDataSource());
        try {
            return jdbcTemplate.executeQuery(Query.LIST_WEBHOOK_ENDPOINTS,
                    (resultSet, rowNumber) -> resultSet.getString(Column.ENDPOINT),
                    preparedStatement -> preparedStatement.setInt(Column.TENANT_ID, tenantId));
        } catch (DataAccessException e) {
            throw WebhookManagementExceptionHandler.handleServerException(
                    ErrorMessage.ERROR_CODE_WEBHOOK_ENDPOINT_LIST_ERROR, e,
     ...
     */
    private void addWebhookEventsInternal(Integer webhookId, List<String> events) throws DataAccessException {
     ...
     */
    private List<String> listWebhookEventsInternal(Integer webhookId) throws DataAccessException {

        NamedJdbcTemplate jdbcTemplate = new NamedJdbcTemplate(IdentityDatabaseUtil.getDataSource());
        return jdbcTemplate.executeQuery(Query.LIST_WEBHOOK_EVENTS,
                (resultSet, rowNumber) -> new String(
                        resultSet.getString(Column.EVENT_NAME)),
                preparedStatement -> {
                    preparedStatement.setInt(Column.WEBHOOK_ID, webhookId);
                });
    }
    ...
    private Webhook mapWebhook(ResultSet resultSet) throws SQLException {
        ...
        webhook.setVersion(resultSet.getString(Column.VERSION));
        ...
        webhook.setEventSchemaVersion(resultSet.getString(Column.EVENT_SCHEMA_VERSION));
        ...
        return jdbcTemplate.withTransaction(template -> {
            template.executeUpdate(Query.CREATE_WEBHOOK,
                    preparedStatement -> {
                        ...
                        preparedStatement.setString(Column.VERSION, webhook.getVersion());
                        ...
                        preparedStatement.setString(Column.EVENT_SCHEMA_VERSION, webhook.getEventSchemaVersion());
                        preparedStatement.setString(Column.STATUS, webhook.getStatus());
                        preparedStatement.setInt(Column.TENANT_ID, tenantId);
                    });

            // Return the internal ID of the created webhook
