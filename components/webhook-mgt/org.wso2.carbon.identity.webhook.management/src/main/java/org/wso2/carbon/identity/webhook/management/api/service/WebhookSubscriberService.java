...
/**
 * Service implementation for webhook subscription operations.
 * This class manages webhook subscribers and delegates subscription operations to appropriate subscribers.
 */
public class WebhookSubscriberService {

    private static final Log LOG = LogFactory.getLog(WebhookSubscriberService.class);

    /**
     * Subscribe a webhook with external systems.
     * This method delegates to appropriate webhook subscribers.
     *
    ...
    public boolean subscribe(Webhook webhook, String tenantDomain) throws WebhookMgtException {
        ...
        List<WebhookSubscriber> subscribers =
                WebhookManagementComponentServiceHolder.getInstance().getWebhookSubscribers();
        if (subscribers.isEmpty()) {
            LOG.warn("No webhook subscribers registered. Webhook subscription will not be processed " +
                    "by external systems.");
            return true; // Return true as we'll still store the webhook in our database
        }
    ...
    /**
     * Unsubscribe a webhook from external systems.
     * This method delegates to appropriate webhook subscribers.
     *
    ...
    public boolean unsubscribe(Webhook webhook, String tenantDomain) throws WebhookMgtException {
        ...
        if (subscribers.isEmpty()) {
            LOG.warn("No webhook subscribers registered. Webhook unsubscription will not be processed " +
                    "by external systems.");
            return true; // Return true as there's nothing to do
        }
        for (WebhookSubscriber subscriber : subscribers) {
            try {
                boolean success =
