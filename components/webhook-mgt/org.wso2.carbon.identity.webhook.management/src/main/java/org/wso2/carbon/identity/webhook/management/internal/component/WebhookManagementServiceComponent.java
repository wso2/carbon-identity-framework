...
public class WebhookManagementServiceComponent {
    ...
    protected void activate(ComponentContext context) {
        ...
        try {
            ...
            WebhookManagementComponentServiceHolder.getInstance().setWebhookSubscriberService(
                    new WebhookSubscriberService());
    ...
    @Reference(
            ...
            service = WebhookSubscriber.class,
            cardinality = ReferenceCardinality.MULTIPLE,
            policy = ReferencePolicy.DYNAMIC,
            unbind = "unregisterSubscriber"
    )
    protected void registerSubscriber(WebhookSubscriber subscriber) {

        if (LOG.isDebugEnabled()) {
            LOG.debug("Registering webhook subscriber: " + subscriber.getName());
        }
        WebhookManagementComponentServiceHolder.getInstance().addWebhookSubscriber(subscriber);
    }
     ...
     * @param subscriber WebhookSubscriber implementation.
     */
    protected void unregisterSubscriber(WebhookSubscriber subscriber) {

        if (LOG.isDebugEnabled()) {
            LOG.debug("Unregistering webhook subscriber: " + subscriber.getName());
        }
        WebhookManagementComponentServiceHolder.getInstance().removeWebhookSubscriber(subscriber);
    }
}
