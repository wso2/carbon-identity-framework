...
package org.wso2.carbon.identity.fraud.detectors.core.util;
...
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.ACCOUNT_CONFIRMATION_TEMPLATE_TYPE;
...
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.ExecutionStatus.SUCCESS;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.INTERNAL_EVENT_NAME;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.ADMIN_FORCED_PASSWORD_RESET_VIA_EMAIL_LINK;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.ADMIN_FORCED_PASSWORD_RESET_VIA_OTP;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.ASK_PASSWORD;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.EMAIL_VERIFICATION_ON_UPDATE;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.MOBILE_VERIFICATION_ON_UPDATE;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.NOTIFICATION_BASED_PW_RECOVERY;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.RecoveryScenarios.NOTIFICATION_BASED_PW_RECOVERY_OFFLINE_INVITE;
import static org.wso2.carbon.identity.fraud.detectors.core.constant.FraudDetectorConstants.TEMPLATE_TYPE;
...
public class EventUtil {
     ...
     */
    public static FraudDetectorConstants.FraudDetectionEvents resolveFraudDetectionEvent(Event event)
            throws UnsupportedFraudDetectionEventException {

        if (isLoginEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.LOGIN;
        } else if (isLogoutEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.LOGOUT;
        } else if (isPostUserCreationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.POST_USER_CREATION;
        } else if (isPostPasswordUpdateEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.POST_UPDATE_PASSWORD;
        } else if (isPasswordUpdateNotificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.PRE_UPDATE_PASSWORD_NOTIFICATION;
        } else if (isSelfRegistrationVerificationNotificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.SELF_REGISTRATION_VERIFICATION_NOTIFICATION;
        } else if (isPostSelfRegistrationVerificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.POST_SELF_REGISTRATION_VERIFICATION;
        } else if (isUserAttributeUpdateVerificationNotificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.USER_ATTRIBUTE_UPDATE_VERIFICATION_NOTIFICATION;
        } else if (isPostUserAttributeUpdateVerificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.POST_USER_ATTRIBUTE_UPDATE_VERIFICATION;
        } else if (isAuthenticationStepVerificationEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.AUTHENTICATION_STEP_NOTIFICATION_VERIFICATION;
        } else if (isPostUserProfileUpdateEvent(event)) {
            return FraudDetectorConstants.FraudDetectionEvents.POST_UPDATE_USER_PROFILE;
        } else {
     ...
     */
    public static void handleEvent(Event event, FraudDetectorConstants.FraudDetectionEvents fraudDetectionEvent,
        ...
        Map<String, IdentityFraudDetector> fraudDetectors =
                IdentityFraudDetectorDataHolder.getInstance().getIdentityFraudDetectors();
    ...
    public static void handleResponse(IdentityFraudDetector fraudDetector, FraudDetectorResponseDTO responseDTO) {
        ...
        if (SKIPPED.equals(responseDTO.getStatus())) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Request to the fraud detector: " + fraudDetectorName + " was skipped as it cannot " +
                        "handle the request. Event name: " + eventName);
            }
        } else if (SUCCESS.equals(responseDTO.getStatus())) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Successfully sent request to the fraud detector: " + fraudDetectorName +
            ...
            LOG.error("Error occurred while processing the request to the fraud detector: " + fraudDetectorName
                    + ". Event name: " + eventName + ". Error Type: " + errorType + ". Error Reason: " + errorReason);
        } else if (FAILURE.equals(responseDTO.getStatus())) {
            LOG.error("Request to the fraud detector: " + fraudDetectorName + " failed."
                    + " Event name: " + eventName);
        }
    }
    ...
    private static UserRealm getUserRealm(String tenantDomain) throws UserStoreException {
        ...
        RealmService realmService = IdentityFraudDetectorDataHolder.getInstance().getRealmService();
