...
public class APIClient {
            ...
            try (CloseableHttpResponse response = httpClient.execute(request)) {
                ...
                if (LoggerUtils.isDiagnosticLogsEnabled()) {
                    DiagnosticLog.DiagnosticLogBuilder diagLogBuilder = new DiagnosticLog.DiagnosticLogBuilder(
                            ...
                            FrameworkConstants.LogConstants.ActionIDs.SEND_ACTION_REQUEST);
                    diagLogBuilder
                            .resultMessage("External endpoint " + request.getURI() + " for action execution seems to be "
                                    + "unavailable. Retrying api call attempt " + retryCount)
                ...
                if (LoggerUtils.isDiagnosticLogsEnabled()) {
                    ...
                    diagLogBuilder
                            .resultMessage("Request for external endpont " + request.getURI() + " for action is " +
                                    "timed out. Retrying api call attempt " + retryCount)
                            .logDetailLevel(DiagnosticLog.LogDetailLevel.APPLICATION)
                            .resultStatus(DiagnosticLog.ResultStatus.SUCCESS)
                            .build();
                    LoggerUtils.triggerDiagnosticLogEvent(diagLogBuilder);
        ...
        switch (statusCode) {
            ...
            case HttpStatus.SC_GATEWAY_TIMEOUT:
                actionInvocationResponseBuilder.errorLog(
                        "Failed to execute the action request. Received: " + statusCode + " status code");
                actionInvocationResponseBuilder.retry(true);
                break;
            default:
                actionInvocationResponseBuilder.errorLog("Unexpected response received with " + statusCode
                        + " status code.");
                break;
        }
    ...
    private void handleSuccess(ActionInvocationResponse.Builder builder, HttpEntity entity, int statusCode) {
        ...
        try {
            ...
            builder.errorLog("Unexpected error occured on response for status code: " + statusCode + ". "
                    + e.getMessage());
        }
    }
    ...
    private void handleClientError(ActionInvocationResponse.Builder builder, HttpEntity entity, int statusCode) {
        ...
        try {
            ...
            if (errorResponse != null) {
                ...
                builder.errorLog("Failed to execute the action request for received " + statusCode
                        + " error code due to no error response is available.");
            }
        } catch (ActionInvocationException e) {
            //todo: add to diagnostic logs
            LOG.debug("JSON payload received for status code: " + statusCode +
                    " is not of the expected error response format. ", e);
            builder.errorLog("Failed to execute the action request for received " + statusCode + " error code. "
                    + e.getMessage());
        }
    }
