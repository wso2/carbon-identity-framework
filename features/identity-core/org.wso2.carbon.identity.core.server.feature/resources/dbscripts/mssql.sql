...
CREATE TABLE IDN_OIDC_JTI (
  JWT_ID VARCHAR(255),
  EXP_TIME DATETIME DEFAULT 0,
  TIME_CREATED DATETIME DEFAULT 0,
  PRIMARY KEY (JWT_ID)
);

IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[IDP_OIDC_PROPERTY]') AND TYPE IN (N'U'))
...
IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[IDN_OIDC_REQ_OBJECT_CLAIM_VALUES]') AND TYPE IN (N'U'))
