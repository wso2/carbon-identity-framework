...
IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[VC_CONFIG]') AND TYPE IN (N'U'))
CREATE TABLE VC_CONFIG (
    ...
    EXPIRES_IN INTEGER,
    PRIMARY KEY (ID),
    CONSTRAINT VC_CONFIG_TENANT_ID_UNQ UNIQUE (ID, TENANT_ID),
    CONSTRAINT VC_CONFIG_TENANT_IDENTIFIER_UNQ UNIQUE (TENANT_ID, IDENTIFIER),
    CONSTRAINT VC_CONFIG_TENANT_DISPLAY_NAME_UNQ UNIQUE (TENANT_ID, DISPLAY_NAME)
);

IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[VC_CONFIG_CLAIM]') AND TYPE IN (N'U'))
CREATE TABLE VC_CONFIG_CLAIM (
    ID INTEGER IDENTITY,
    CONFIG_ID CHAR(36) NOT NULL,
    CLAIM_URI VARCHAR(255) NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT VC_CONFIG_CLAIM_CFG_CLAIM_UNQ UNIQUE (CONFIG_ID, CLAIM_URI),
    CONSTRAINT FK_VC_CONFIG_CLAIM_CONFIG FOREIGN KEY (CONFIG_ID) REFERENCES VC_CONFIG (ID) ON DELETE CASCADE
);

IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[VC_OFFER]') AND TYPE IN (N'U'))
CREATE TABLE VC_OFFER (
    ...
    CONSTRAINT VC_OFFER_TENANT_ID_UNQ UNIQUE (OFFER_ID, TENANT_ID),
    CONSTRAINT VC_OFFER_TENANT_DISPLAY_NAME_UNQ UNIQUE (TENANT_ID, DISPLAY_NAME)
);

IF NOT  EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[VC_OFFER_CREDENTIAL_CONFIG]') AND TYPE IN (N'U'))
CREATE TABLE VC_OFFER_CREDENTIAL_CONFIG (
    ...
    TENANT_ID INTEGER NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT VC_OFFER_CONFIG_OFFER_CONFIG_UNQ UNIQUE (OFFER_ID, CONFIG_ID),
    CONSTRAINT FK_VC_OFFER_CONFIG_OFFER FOREIGN KEY (OFFER_ID, TENANT_ID) REFERENCES VC_OFFER (OFFER_ID, TENANT_ID) ON DELETE CASCADE,
    CONSTRAINT FK_VC_OFFER_CONFIG_CONFIG FOREIGN KEY (CONFIG_ID, TENANT_ID) REFERENCES VC_CONFIG (ID, TENANT_ID) ON DELETE CASCADE
);
