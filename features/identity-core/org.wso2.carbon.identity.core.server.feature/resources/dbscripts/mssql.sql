...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[DBO].[SP_IDP_ATTR]') AND TYPE in (N'U'))
...
IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[IDN_NOTIF_TEMPLATE_TYPE]') AND TYPE IN (N'U'))
CREATE TABLE IDN_NOTIF_TEMPLATE_TYPE (
    ID CHAR(36) NOT NULL,
    DISPLAY_NAME VARCHAR(255) NOT NULL,
    TEMPLATE_TYPE VARCHAR(255) NOT NULL,
    CHANNEL VARCHAR(255) NOT NULL,
    TENANT_UUID CHAR(36) NOT NULL,
    CONSTRAINT PK_IDN_NOTIF_TEMPLATE_TYPE PRIMARY KEY (ID),
    CONSTRAINT UNIQUE_TEMPLATE_TYPE_CHANNEL_TENANT_UUID UNIQUE (TEMPLATE_TYPE, CHANNEL, TENANT_UUID)
);

IF NOT EXISTS (SELECT * FROM SYS.OBJECTS WHERE OBJECT_ID = OBJECT_ID(N'[dbo].[IDN_NOTIF_TEMPLATE]') AND TYPE IN (N'U'))
CREATE TABLE IDN_NOTIF_TEMPLATE (
    TEMPLATE_TYPE_ID CHAR(36) NOT NULL,
    LOCALE VARCHAR(255) NOT NULL,
    CONTENT TEXT NOT NULL,
    CONTENT_TYPE VARCHAR(255) NULL,
    CONSTRAINT PK_IDN_NOTIF_TEMPLATE PRIMARY KEY (TEMPLATE_TYPE_ID, LOCALE),
    FOREIGN KEY (TEMPLATE_TYPE_ID) REFERENCES IDN_NOTIF_TEMPLATE_TYPE(ID) ON DELETE CASCADE
);
