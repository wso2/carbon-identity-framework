...
CREATE TRIGGER IDN_CLAIM_MAPPING_DELETE_TRIGGER ON IDN_CLAIM INSTEAD OF DELETE AS IF OBJECT_ID('IDN_CLAIM_MAPPING_DELETE_TRIGGER', 'TR') IS NOT NULL BEGIN
	DELETE IDN_CLAIM_MAPPING WHERE EXT_CLAIM_ID IN (SELECT ID FROM deleted)  OR MAPPED_LOCAL_CLAIM_ID IN (SELECT ID FROM DELETED);
	DELETE ID_CLAIM WHERE ID IN (SELECT ID FROM deleted);
END
