...
CREATE TABLE IDN_OAUTH2_TOKEN_CLAIMS (
            ...
            PRIMARY KEY (ID),
            CONSTRAINT IDN_OAUTH2_TOKEN_CLAIMS_CONSTRAINT UNIQUE (TENANT_ID, CONSUMER_KEY, CLAIM_URI),
            FOREIGN KEY (TENANT_ID, CONSUMER_KEY) REFERENCES IDN_OAUTH_CONSUMER_APPS(TENANT_ID, CONSUMER_KEY) ON DELETE CASCADE)
/
CREATE IDN_OAUTH2_TOKEN_CLAIMS_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE TRIGGER IDN_OAUTH2_TOKEN_CLAIMS_TRIG NO CASCADE
            BEFORE INSERT
            ON IDN_OAUTH2_TOKEN_CLAIMS
            REFERENCING NEW AS NEW
            FOR EACH ROW MODE DB2SQL
                BEGIN ATOMIC
                    SET (NEW.ID) = (NEXTVAL FOR IDN_OAUTH2_TOKEN_CLAIMS_SEQ);
                END
/
