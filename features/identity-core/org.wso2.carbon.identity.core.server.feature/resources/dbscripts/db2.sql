...
CREATE TABLE IDN_DIALECT(
  IDN_ID INTEGER NOT NULL,
  IDN_DIALECT_URI VARCHAR(255) NOT NULL,
  IDN_TENANT_ID INTEGER DEFAULT 0,
  UNIQUE(IDN_DIALECT_URI, IDN_TENANT_ID),
  PRIMARY KEY (IDN_ID)
)
/

CREATE SEQUENCE IDN_DIALECT_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TRIGGER IDN_DIALECT_TRIG NO CASCADE
BEFORE INSERT
ON IDN_DIALECT
REFERENCING NEW AS NEW
FOR EACH ROW MODE DB2SQL
  BEGIN ATOMIC
    SET (NEW.IDN_ID) = (NEXTVAL FOR IDN_DIALECT_SEQ);
  END
/

CREATE TABLE IDN_CLAIM(
  IDN_ID INTEGER NOT NULL,
  IDN_DIALECT_ID INTEGER NOT NULL,
  IDN_CLAIM_URI VARCHAR(255) NOT NULL,
  IDN_TENANT_ID INTEGER DEFAULT 0,
  FOREIGN KEY(IDN_DIALECT_ID, IDN_TENANT_ID) REFERENCES IDN_DIALECT(IDN_ID, IDN_TENANT_ID) ON DELETE CASCADE,
  PRIMARY KEY (IDN_ID)
)
/
...
FOR EACH ROW MODE DB2SQL
  BEGIN ATOMIC
    SET (NEW.IDN_ID) = (NEXTVAL FOR IDN_CLAIM_SEQ);
  END
/

CREATE TABLE IDN_CLAIM_META_DATA (
  IDN_ID INTEGER NOT NULL,
  IDN_CLAIM_ID INTEGER NOT NULL,
  IDN_TENANT_ID INTEGER DEFAULT 0,
  IDN_KEY VARCHAR(255) DEFAULT NULL,
  IDN_VALUE VARCHAR(255) DEFAULT NULL,
  PRIMARY KEY (IDN_ID),
  FOREIGN KEY (IDN_CLAIM_ID) REFERENCES IDN_CLAIM (IDN_ID) ON DELETE CASCADE
)
/

CREATE SEQUENCE IDN_CLAIM_META_DATA_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TRIGGER IDN_CLAIM_META_DATA_TRIG NO CASCADE
BEFORE INSERT
ON IDN_CLAIM_META_DATA
REFERENCING NEW AS NEW
FOR EACH ROW MODE DB2SQL
  BEGIN ATOMIC
    SET (NEW.IDN_ID) = (NEXTVAL FOR IDN_CLAIM_META_DATA_SEQ);
  END
/

CREATE TABLE IDN_CLAIM_MAPPING (
  IDN_ID INTEGER NOT NULL,
  IDN_LOCAL_CLAIM_ID INTEGER NOT NULL,
  IDN_TENANT_ID INTEGER DEFAULT 0,
  IDN_ADDITIONAL_CLAIM_ID INTEGER NOT NULL,
  PRIMARY KEY (IDN_ID),
  FOREIGN KEY (IDN_LOCAL_CLAIM_ID) REFERENCES IDN_CLAIM (IDN_ID) ON DELETE CASCADE,
  FOREIGN KEY (IDN_ADDITIONAL_CLAIM_ID) REFERENCES IDN_CLAIM (IDN_ID) ON DELETE CASCADE
)
/
...
FOR EACH ROW MODE DB2SQL
  BEGIN ATOMIC
    SET (NEW.IDN_ID) = (NEXTVAL FOR IDN_CLAIM_MAPPING_SEQ);
  END
/
...
CREATE TABLE IDN_CLAIM_MAPPED_ATTRIBUTE (
  ...
  IDN_CLAIM_ID INTEGER NOT NULL,
  IDN_TENANT_ID INTEGER DEFAULT 0,
  IDN_MAPPED_ATTRIBUTE_DOMAIN VARCHAR(255) DEFAULT NULL,
  IDN_MAPPED_ATTRIBUTE VARCHAR(255) DEFAULT NULL,
  PRIMARY KEY (IDN_ID),
  FOREIGN KEY (IDN_CLAIM_ID) REFERENCES IDN_CLAIM (IDN_ID) ON DELETE CASCADE
)
/
...
FOR EACH ROW MODE DB2SQL
  BEGIN ATOMIC
    SET (NEW.IDN_ID) = (NEXTVAL FOR IDN_CLAIM_MAPPED_ATTRIBUTE_SEQ);
  END
/
