...
CREATE TABLE SP_APP (
	    ...
	    IS_CONSENT_MGT_ENABLED CHAR(1) DEFAULT '1',
        PRIMARY KEY (ID))
/
...
CREATE TABLE SP_CONSENT_PURPOSE_ASSOC (
  ID            INTEGER NOT NULL,
  TENANT_ID     INTEGER NOT NULL,
  APP_ID        INTEGER NOT NULL,
  ...
  PRIMARY KEY (ID),
  UNIQUE (APP_ID, PURPOSE_ID, TENANT_ID),
  FOREIGN KEY (APP_ID) REFERENCES SP_APP(ID) ON DELETE CASCADE)
/
CREATE SEQUENCE SP_CONSENT_PUR_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE TRIGGER SP_CONSENT_PUR_TRIG NO CASCADE
            BEFORE INSERT
            ON SP_CONSENT_PURPOSE_ASSOC
            REFERENCING NEW AS NEW
            FOR EACH ROW MODE DB2SQL
                BEGIN ATOMIC
                    SET (NEW.ID) = (NEXTVAL FOR SP_CONSENT_PUR_SEQ);
                END
/
