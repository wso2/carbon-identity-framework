...
/
CREATE TRIGGER IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT_TRIG
            BEFORE INSERT
            ON IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT
            REFERENCING NEW AS NEW
            FOR EACH ROW MODE DB2SQL
            BEGIN ATOMIC
                SET (NEW.ID) = (NEXTVAL FOR IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT_SEQ);
            END;
/
...
CREATE TRIGGER DELETE_FROM_AUTHORIZED_SCOPE_AFTER_SP_APP_DELETE_TRIGGER AFTER DELETE ON SP_APP
...
CREATE TRIGGER DELETE_FROM_AUTHORIZED_SCOPE_AFTER_API_RESOURCE_DELETE_TRIGGER AFTER DELETE ON API_RESOURCE
            REFERENCING OLD AS OLD FOR EACH ROW MODE DB2SQL
            BEGIN ATOMIC
                DELETE FROM AUTHORIZED_SCOPE WHERE APP_ID = OLD.ID;
            END
/
