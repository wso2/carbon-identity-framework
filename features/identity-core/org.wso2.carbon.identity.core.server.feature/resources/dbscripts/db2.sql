...
/
CREATE SEQUENCE TEMPLATE_ID START WITH 1 INCREMENT BY 1 NOCACHE
/

CREATE TRIGGER IDN_TEMPLATE_MGT NO CASCADE
    BEFORE INSERT
    ON IDN_TEMPLATE_MGT
    REFERENCING NEW AS NEW
    FOR EACH ROW MODE DB2SQL
      BEGIN ATOMIC
        SET (NEW.TEMPLATE_ID) = (NEXTVAL FOR IDN_TEMPLATE_MGT_SEQ);
      END
/
