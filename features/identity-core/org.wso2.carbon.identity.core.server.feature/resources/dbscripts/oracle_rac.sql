...
CREATE TABLE IDN_OIDC_REQUEST_OBJECT (
  ID INTEGER,
  CONSUMER_KEY_ID INTEGER ,
  CODE_ID VARCHAR(255) ,
  TOKEN_ID VARCHAR(255) ,
...
/
CREATE SEQUENCE IDN_OIDC_REQUEST_OBJECT_SEQ START WITH 1 INCREMENT BY 1 CACHE 20 ORDER
/
CREATE OR REPLACE TRIGGER IDN_OIDC_REQUEST_OBJECT_TRIG
            BEFORE INSERT
            ON IDN_OIDC_REQUEST_OBJECT
            REFERENCING NEW AS NEW
            FOR EACH ROW
               BEGIN
                   SELECT IDN_OIDC_REQUEST_OBJECT_SEQ.nextval INTO :NEW.ID FROM dual;
               END;
/
