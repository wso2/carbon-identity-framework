...
CREATE TABLE IDN_DIALECT(
  ...
  PRIMARY KEY (IDN_ID, IDN_TENANT_ID)
)
/

CREATE SEQUENCE IDN_DIALECT_SEQ START WITH 1 INCREMENT BY 1 CACHE 20 ORDER
/
CREATE OR REPLACE TRIGGER IDN_DIALECT_TRIG
BEFORE INSERT
ON IDN_DIALECT
REFERENCING NEW AS NEW
FOR EACH ROW
  BEGIN
    SELECT IDN_DIALECT_SEQ.nextval
    INTO :NEW.IDN_ID
    FROM dual;
  END;
/
...
CREATE SEQUENCE IDN_CLAIM_META_DATA_SEQ START WITH 1 INCREMENT BY 1 CACHE 20 ORDER
/
CREATE OR REPLACE TRIGGER IDN_CLAIM_META_DATA_TRIG
BEFORE INSERT
ON IDN_CLAIM_META_DATA
REFERENCING NEW AS NEW
FOR EACH ROW
  BEGIN
    SELECT IDN_CLAIM_META_DATA_SEQ.nextval
    INTO :NEW.IDN_ID
    FROM dual;
  END;
/
