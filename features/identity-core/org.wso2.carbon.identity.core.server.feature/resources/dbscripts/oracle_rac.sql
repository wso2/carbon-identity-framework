...
CREATE OR REPLACE TRIGGER IDN_ORG_USER_INVITATION_TRIG
            ...
            END;
/
CREATE TABLE IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT (
	ID INTEGER NOT NULL,
	INVITATION_ID VARCHAR(40) NOT NULL,
	...
	FOREIGN KEY (APPLICATION_ID) REFERENCES SP_APP(UUID) ON DELETE CASCADE
)
/
CREATE SEQUENCE IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT_TRIG
            BEFORE INSERT
            ON IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT
            REFERENCING NEW AS NEW
            FOR EACH ROW
            BEGIN
                SELECT IDN_ORG_USER_INVITE_ROLE_ASSIGNMENT_SEQ.nextval INTO :NEW.ID FROM dual;
            END;
/
CREATE TABLE IDN_ORG_USER_ASSOCIATION(
	SHARED_USER_ID VARCHAR(255) NOT NULL,
	SUB_ORG_ID VARCHAR(36) NOT NULL,
