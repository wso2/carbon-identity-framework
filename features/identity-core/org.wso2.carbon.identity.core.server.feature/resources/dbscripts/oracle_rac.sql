...
/
CREATE SEQUENCE VC_CONFIG_CURSOR_KEY_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER VC_CONFIG_CURSOR_KEY_TRIG
    BEFORE INSERT
    ON VC_CONFIG
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT VC_CONFIG_CURSOR_KEY_SEQ.nextval INTO :NEW.CURSOR_KEY FROM dual;
    END;
/

CREATE TABLE VC_CONFIG_CLAIM (
    ID INTEGER NOT NULL,
    CONFIG_ID CHAR(36) NOT NULL,
    CLAIM_URI VARCHAR(255) NOT NULL,
    PRIMARY KEY (ID),
    UNIQUE (CONFIG_ID, CLAIM_URI),
    FOREIGN KEY (CONFIG_ID) REFERENCES VC_CONFIG (ID) ON DELETE CASCADE
)
/
CREATE SEQUENCE VC_CONFIG_CLAIM_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER VC_CONFIG_CLAIM_TRIG
    BEFORE INSERT
    ON VC_CONFIG_CLAIM
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT VC_CONFIG_CLAIM_SEQ.nextval INTO :NEW.ID FROM dual;
    END;
/

CREATE TABLE VC_OFFER (
...
/
CREATE SEQUENCE VC_OFFER_CURSOR_KEY_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER VC_OFFER_CURSOR_KEY_TRIG
    BEFORE INSERT
    ON VC_OFFER
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT VC_OFFER_CURSOR_KEY_SEQ.nextval INTO :NEW.CURSOR_KEY FROM dual;
    END;
/

CREATE TABLE VC_OFFER_CREDENTIAL_CONFIG (
    ID INTEGER NOT NULL,
    OFFER_ID CHAR(36) NOT NULL,
    CONFIG_ID CHAR(36) NOT NULL,
    TENANT_ID INTEGER NOT NULL,
    PRIMARY KEY (ID),
    UNIQUE (OFFER_ID, CONFIG_ID),
    FOREIGN KEY (OFFER_ID, TENANT_ID) REFERENCES VC_OFFER (OFFER_ID, TENANT_ID) ON DELETE CASCADE,
    FOREIGN KEY (CONFIG_ID, TENANT_ID) REFERENCES VC_CONFIG (ID, TENANT_ID) ON DELETE CASCADE
)
/
CREATE SEQUENCE VC_OFFER_CREDENTIAL_CONFIG_SEQ START WITH 1 INCREMENT BY 1 NOCACHE
/
CREATE OR REPLACE TRIGGER VC_OFFER_CREDENTIAL_CONFIG_TRIG
    BEFORE INSERT
    ON VC_OFFER_CREDENTIAL_CONFIG
    REFERENCING NEW AS NEW
    FOR EACH ROW
    BEGIN
        SELECT VC_OFFER_CREDENTIAL_CONFIG_SEQ.nextval INTO :NEW.ID FROM dual;
    END;
/
