...
BEGIN
    ...
    SET backupTables = FALSE; -- SET IF OAUTH PAR TABLE NEEDS TO BACKED-UP BEFORE DELETE [DEFAULT : FALSE] , WILL DROP THE PREVIOUS BACKUP TABLES IN NEXT ITERATION
    ...
    CONFIRMATION_CHUNK_LOOP : WHILE (chunkCount > 0) DO
        ...
        CREATE TABLE IDN_OAUTH_PAR_CHUNK_TMP AS SELECT REQ_URI_REF FROM IDN_OAUTH_PAR WHERE (cleanUpDateTimeLimit > TIME_CREATED) LIMIT chunkSize;
