...
DECLARE isCibaTableExists INTEGER DEFAULT 0;
...
TABLE_NAME IN ('IDN_OAUTH2_ACCESS_TOKEN','IDN_OAUTH2_ACCESS_TOKEN_SCOPE','IDN_OIDC_REQ_OBJECT_REFERENCE','IDN_OIDC_REQ_OBJECT_CLAIMS','IDN_OIDC_REQ_OBJ_CLAIM_VALUES','IDN_OAUTH2_AUTHORIZATION_CODE','IDN_OAUTH2_CIBA_AUTH_CODE');
...
SELECT COUNT(1) INTO isCibaTableExists FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'IDN_OAUTH2_CIBA_AUTH_CODE' AND TABLE_SCHEMA IN (SELECT DATABASE());
...
THEN
    ...
    IF (isCibaTableExists > 0)
    THEN
        IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP' and TABLE_SCHEMA in (SELECT DATABASE())))
        THEN
            IF (enableLog AND logLevel IN ('TRACE')) THEN
            SELECT 'CREATING AUDIT TABLE AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP .. !';
            END IF;
            CREATE TABLE AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP SELECT * FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE 1 = 2;
        ELSE
            IF (enableLog AND logLevel IN ('TRACE')) THEN
            SELECT 'USING AUDIT TABLE AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP ..!';
            END IF;
        END IF;
    END IF;
END IF;
...
TOKE_CHUNK_LOOP: REPEAT
      ...
      DROP TEMPORARY TABLE IF EXISTS CHUNK_IDN_OAUTH2_ACCESS_TOKEN;
      ...
      TOKE_BATCH_LOOP: REPEAT
            ...
            DROP TEMPORARY TABLE IF EXISTS BATCH_IDN_OAUTH2_ACCESS_TOKEN;
...
CODE_CHUNK_LOOP: REPEAT
        ...
        DROP TEMPORARY TABLE IF EXISTS CHUNK_IDN_OAUTH2_AUTHORIZATION_CODE;
        ...
        CODE_BATCH_LOOP: REPEAT
                ...
                DROP TEMPORARY TABLE IF EXISTS BATCH_IDN_OAUTH2_AUTHORIZATION_CODE;
...
-- ------------------------------------------------------
-- CALCULATING CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE
-- ------------------------------------------------------
IF (isCibaTableExists > 0)
THEN
...
IF (enableLog)
THEN
    SELECT 'CALCULATING CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE .... !' AS 'INFO LOG';

    IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
    THEN
    SELECT  COUNT(1) into rowcount FROM IDN_OAUTH2_CIBA_AUTH_CODE;
    SELECT 'TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE BEFORE DELETE' AS 'DEBUG LOG', rowcount;
    END IF;

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
    SELECT COUNT(1) into @cleaupCount FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND deleteTillTime > DATE_ADD(ISSUED_TIME, INTERVAL +(EXPIRES_IN) SECOND));
    SELECT 'TOTAL CIBA AUTH CODES SHOULD BE DELETED FROM IDN_OAUTH2_CIBA_AUTH_CODE' AS 'TRACE LOG', @cleaupCount;
    END IF;

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
    SET rowcount  = (rowcount - @cleaupCount);
    SELECT 'TOTAL CIBA AUTH CODES SHOULD BE RETAIN IN IDN_OAUTH2_CIBA_AUTH_CODE' AS 'TRACE LOG', rowcount;
    END IF;
END IF;
...
IF (enableLog)
THEN
SELECT 'CIBA AUTH CODE DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE STARTED .... !' AS 'INFO LOG';
END IF;

CIBA_CHUNK_LOOP: REPEAT

        DROP TEMPORARY TABLE IF EXISTS CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE;

        CREATE TEMPORARY TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE SELECT AUTH_CODE_KEY FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND deleteTillTime > DATE_ADD(ISSUED_TIME, INTERVAL +(EXPIRES_IN) SECOND)) LIMIT chunkSize;

        SELECT COUNT(1) INTO @chunkCount FROM CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE;

        IF (@chunkCount<checkCount )
        THEN
        LEAVE CIBA_CHUNK_LOOP;
        END IF;

        IF (enableLog AND logLevel IN ('TRACE'))
        THEN
        SELECT 'PROCCESING CHUNK IDN_OAUTH2_CIBA_AUTH_CODE STARTED .... !' AS 'TRACE LOG',@chunkCount ;
        END IF;

        CREATE INDEX IDX_CHK_IDN_OAUTH2_CIBA_AUTH_CODE ON CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE (AUTH_CODE_KEY);

        IF (enableAudit)
        THEN
        INSERT INTO AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP SELECT CIBA.* FROM IDN_OAUTH2_CIBA_AUTH_CODE AS CIBA INNER JOIN CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE AS CHK ON CIBA.AUTH_CODE_KEY = CHK.AUTH_CODE_KEY;
        END IF;

        CIBA_BATCH_LOOP: REPEAT

                DROP TEMPORARY TABLE IF EXISTS BATCH_IDN_OAUTH2_CIBA_AUTH_CODE;

                CREATE TEMPORARY TABLE BATCH_IDN_OAUTH2_CIBA_AUTH_CODE SELECT AUTH_CODE_KEY FROM CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE LIMIT batchSize;

                SELECT COUNT(1) INTO @batchCount FROM BATCH_IDN_OAUTH2_CIBA_AUTH_CODE;

                IF (@batchCount=0 )
                THEN
                LEAVE CIBA_BATCH_LOOP;
                END IF;

                IF (enableLog AND logLevel IN ('TRACE'))
                THEN
                SELECT 'BATCH DELETE STARTED ON IDN_OAUTH2_CIBA_AUTH_CODE:' AS 'TRACE LOG', @batchCount;
                END IF;

                DELETE A
                FROM IDN_OAUTH2_CIBA_AUTH_CODE AS A
                INNER JOIN BATCH_IDN_OAUTH2_CIBA_AUTH_CODE AS B
                ON A.AUTH_CODE_KEY = B.AUTH_CODE_KEY;

                SELECT row_count() INTO rowCount;

                IF (enableLog)
                THEN
                SELECT 'BATCH DELETE FINISHED ON IDN_OAUTH2_CIBA_AUTH_CODE:' AS 'INFO LOG', rowCount;
                END IF;

                DELETE A
                FROM CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE AS A
                INNER JOIN BATCH_IDN_OAUTH2_CIBA_AUTH_CODE AS B
                ON A.AUTH_CODE_KEY = B.AUTH_CODE_KEY;
...
IF (enableLog)
THEN
SELECT 'CIBA AUTH CODE DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE COMPLETED .... !' AS 'INFO LOG';
END IF;

END IF; -- END OF CIBA TABLE EXISTS CHECK
-- ------------------------------------------------------
...
IF (isCibaTableExists > 0 AND enableLog AND logLevel IN ('DEBUG','TRACE'))
THEN
SELECT 'TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE AFTER DELETE' AS 'DEBUG LOG', COUNT(1) FROM IDN_OAUTH2_CIBA_AUTH_CODE;
END IF;
...
THEN
    ...
    IF (isCibaTableExists > 0)
    THEN
    ANALYZE TABLE IDN_OAUTH2_CIBA_AUTH_CODE;
    END IF;
