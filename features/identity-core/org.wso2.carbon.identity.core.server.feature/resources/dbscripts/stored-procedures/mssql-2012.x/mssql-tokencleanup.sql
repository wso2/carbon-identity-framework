...
SET @enableAudit = 'TRUE'; -- SET TRUE FOR  KEEP TRACK OF ALL THE DELETED TOKENS USING A TABLE    [DEFAULT : TRUE] [# IF YOU ENABLE THIS TABLE BACKUP WILL FORCEFULLY SET TO TRUE]
...
BEGIN
		...
		IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
		BEGIN
		SELECT @cleaupCount = COUNT(1) FROM IDN_OAUTH2_ACCESS_TOKEN WHERE TOKEN_STATE IN ('EXPIRED','INACTIVE','REVOKED') OR
...
BEGIN
		...
		INSERT INTO CHUNK_IDN_OAUTH2_ACCESS_TOKEN (TOKEN_ID) SELECT TOP (@chunkSize) TOKEN_ID FROM IDN_OAUTH2_ACCESS_TOKEN WHERE (TOKEN_STATE IN ('EXPIRED','INACTIVE','REVOKED') OR
		(TOKEN_STATE in('ACTIVE') AND
		(VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND (REFRESH_TOKEN_VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND
		(@deleteTimeLimit > DATEADD(MINUTE, (VALIDITY_PERIOD/60000), TIME_CREATED) AND (@deleteTimeLimit > DATEADD(MINUTE, (REFRESH_TOKEN_VALIDITY_PERIOD/60000), REFRESH_TOKEN_TIME_CREATED)))));
...
-- -------------
	IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
	BEGIN
	SELECT @cleaupCount = COUNT(1) FROM IDN_OAUTH2_AUTHORIZATION_CODE WHERE CODE_ID IN
	...
	AND (((VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND  (@deleteTimeLimit > DATEADD(MINUTE, (VALIDITY_PERIOD/60000), TIME_CREATED))) OR STATE ='INACTIVE');
...
BEGIN
	...
	INSERT INTO CHNK_IDN_OATH_AUTHRIZATN_CODE (CODE_ID) SELECT TOP (@chunkSize) CODE_ID FROM IDN_OAUTH2_AUTHORIZATION_CODE WHERE CODE_ID IN
	(SELECT CODE_ID FROM IDN_OAUTH2_AUTHORIZATION_CODE code WHERE NOT EXISTS (SELECT * FROM IDN_OAUTH2_ACCESS_TOKEN tok where tok.TOKEN_ID = code.TOKEN_ID))
	AND (((VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND (@deleteTimeLimit > DATEADD(MINUTE, (VALIDITY_PERIOD/60000), TIME_CREATED))) OR STATE ='INACTIVE');
