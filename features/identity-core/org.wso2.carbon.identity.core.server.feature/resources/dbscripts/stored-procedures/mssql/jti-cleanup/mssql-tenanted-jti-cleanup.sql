...
BEGIN
	...
	CREATE TABLE CHUNK_IDN_OIDC_JTI (JWT_ID VARCHAR (255),TENANT_ID INTEGER,CONSTRAINT CHNK_IDN_OIDC_JTI_PRI PRIMARY KEY (JWT_ID,TENANT_ID));

	INSERT INTO CHUNK_IDN_OIDC_JTI (JWT_ID,TENANT_ID) SELECT TOP (@chunkSize) JWT_ID,TENANT_ID FROM IDN_OIDC_JTI WHERE (CONVERT(DATE, EXP_TIME) < CONVERT(DATE, CURRENT_TIMESTAMP));
	...
	BEGIN
		...
		CREATE TABLE BATCH_IDN_OIDC_JTI (JWT_ID VARCHAR (255),TENANT_ID INTEGER,CONSTRAINT BATCH_IDN_OIDC_JTI_PRI PRIMARY KEY (JWT_ID,TENANT_ID));

		INSERT INTO BATCH_IDN_OIDC_JTI (JWT_ID,TENANT_ID) SELECT TOP (@batchSize) JWT_ID,TENANT_ID FROM CHUNK_IDN_OIDC_JTI;
		...
		BEGIN
			...
			DELETE FROM IDN_OIDC_JTI WHERE EXISTS (SELECT * FROM BATCH_IDN_OIDC_JTI WHERE (IDN_OIDC_JTI.JWT_ID=BATCH_IDN_OIDC_JTI.JWT_ID AND IDN_OIDC_JTI.TENANT_ID=BATCH_IDN_OIDC_JTI.TENANT_ID));
			...
			DELETE FROM CHUNK_IDN_OIDC_JTI WHERE EXISTS (SELECT * FROM BATCH_IDN_OIDC_JTI WHERE (CHUNK_IDN_OIDC_JTI.JWT_ID=BATCH_IDN_OIDC_JTI.JWT_ID AND CHUNK_IDN_OIDC_JTI.TENANT_ID=BATCH_IDN_OIDC_JTI.TENANT_ID));
