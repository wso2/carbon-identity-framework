...
BEGIN
		...
		IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
		BEGIN
		SELECT @cleaupCount = COUNT(1) FROM IDN_OAUTH2_ACCESS_TOKEN WHERE TOKEN_STATE IN ('INACTIVE','REVOKED') OR
...
BEGIN
		...
		INSERT INTO CHUNK_IDN_OAUTH2_ACCESS_TOKEN (TOKEN_ID) SELECT TOP (@chunkSize) TOKEN_ID FROM IDN_OAUTH2_ACCESS_TOKEN WHERE (TOKEN_STATE IN ('INACTIVE','REVOKED') OR
		(TOKEN_STATE in('EXPIRED','ACTIVE') AND
		(VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND (REFRESH_TOKEN_VALIDITY_PERIOD BETWEEN 0 and @maxValidityPeriod) AND
		(@deleteTimeLimit > DATEADD(MINUTE, (VALIDITY_PERIOD/60000), TIME_CREATED) AND (@deleteTimeLimit > DATEADD(MINUTE, (REFRESH_TOKEN_VALIDITY_PERIOD/60000), REFRESH_TOKEN_TIME_CREATED)))));
...
-- -------------
	...
	AND (VALIDITY_PERIOD < @maxValidityPeriod AND  @deleteTimeLimit > DATEADD(MINUTE, (VALIDITY_PERIOD/60000), TIME_CREATED));
