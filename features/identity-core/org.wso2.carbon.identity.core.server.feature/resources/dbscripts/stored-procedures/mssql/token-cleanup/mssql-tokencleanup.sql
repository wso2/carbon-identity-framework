...
TABLE_NAME IN ('IDN_OAUTH2_ACCESS_TOKEN', 'IDN_OAUTH2_AUTHORIZATION_CODE', 'IDN_OAUTH2_ACCESS_TOKEN_SCOPE','IDN_OIDC_REQ_OBJECT_REFERENCE','IDN_OIDC_REQ_OBJECT_CLAIMS','IDN_OIDC_REQ_OBJ_CLAIM_VALUES','IDN_OAUTH2_CIBA_AUTH_CODE')
...
BEGIN
	...
	IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP'))
	BEGIN
			IF (@enableLog = 1 AND @logLevel IN ('TRACE')) BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] CREATING AUDIT TABLE AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP .. !';
			END
			Select * into dbo.AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP  from  dbo.IDN_OAUTH2_CIBA_AUTH_CODE where 1 =2;
	END
	ELSE
	BEGIN
			IF (@enableLog = 1 AND @logLevel IN ('TRACE')) BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] USING AUDIT TABLE AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP ..!';
			END
	END
END
...
BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN TRANSACTION
				DELETE IDN_OIDC_REQ_OBJECT_REFERENCE where TOKEN_ID in (select TOKEN_ID from BATCH_IDN_OAUTH2_ACCESS_TOKEN);
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN TRANSACTION
			DELETE FROM IDN_OIDC_REQ_OBJECT_REFERENCE where CODE_ID in (select CODE_ID from BATCH_IDN_OATH2_AUTHRIZATN_CDE);
...
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- CALCULATING CIBA AUTH CODES IN IDN_OAUTH2_CIBA_AUTH_CODE
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
IF (@enableLog = 1)
BEGIN
	SELECT '[' + convert(varchar, getdate(), 121) + '] CALCULATING CIBA AUTH CODES IN IDN_OAUTH2_CIBA_AUTH_CODE TABLE .... !';

	IF (@enableLog = 1 AND @logLevel IN ('DEBUG','TRACE'))
	BEGIN
	SELECT @rowcount = COUNT(1) FROM IDN_OAUTH2_CIBA_AUTH_CODE;
	SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE BEFORE DELETE :'+CAST(@rowCount as varchar);
	END

	IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
	BEGIN
	SELECT @cleaupCount = COUNT(1) FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND @deleteTimeLimit > DATEADD(SECOND, EXPIRES_IN, ISSUED_TIME));
	SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL CIBA AUTH CODES SHOULD BE DELETED FROM IDN_OAUTH2_CIBA_AUTH_CODE : '+CAST(@cleaupCount as varchar);
	END

	IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
	BEGIN
	select @rowcount  = (@rowcount - @cleaupCount);
	SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL CIBA AUTH CODES SHOULD BE RETAIN IN IDN_OAUTH2_CIBA_AUTH_CODE : '+CAST(@rowCount as varchar);
	END
END
...
IF (@enableLog = 1)
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] CIBA AUTH CODE DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE STARTED ... !';
END

WHILE (1=1)
BEGIN

	IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE'))
	BEGIN
	DROP TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE;
	END

	CREATE TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE (AUTH_CODE_KEY CHAR(36), CONSTRAINT IDN_CHNK_CIBA_AUTH_CODE_PRI PRIMARY KEY (AUTH_CODE_KEY));

	INSERT INTO CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE (AUTH_CODE_KEY) SELECT TOP (@chunkSize) AUTH_CODE_KEY FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND @deleteTimeLimit > DATEADD(SECOND, EXPIRES_IN, ISSUED_TIME));
	SELECT @chunkCount = @@rowcount;
	...
	IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
	BEGIN
	SELECT '[' + convert(varchar, getdate(), 121) + '] CHUNK TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE CREATED WITH : '+CAST(@chunkCount as varchar);
	END

	IF (@enableAudit = 1)
	BEGIN
	INSERT INTO dbo.AUDITLOG_IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP SELECT CIBA.* FROM IDN_OAUTH2_CIBA_AUTH_CODE CIBA, CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE CHK WHERE CIBA.AUTH_CODE_KEY = CHK.AUTH_CODE_KEY;
	END

	WHILE (1=1)
	BEGIN

		IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'BATCH_IDN_OAUTH2_CIBA_AUTH_CODE'))
		BEGIN
		DROP TABLE BATCH_IDN_OAUTH2_CIBA_AUTH_CODE;
		END

		CREATE TABLE BATCH_IDN_OAUTH2_CIBA_AUTH_CODE (AUTH_CODE_KEY CHAR(36), CONSTRAINT IDN_BATCH_CIBA_AUTH_CODE_PRI PRIMARY KEY (AUTH_CODE_KEY));

		INSERT INTO BATCH_IDN_OAUTH2_CIBA_AUTH_CODE (AUTH_CODE_KEY) SELECT TOP (@batchSize) AUTH_CODE_KEY FROM CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE;
		SELECT @batchCount = @@rowcount;
		...
		BEGIN
			IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
			BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] BATCH DELETE START ON TABLE IDN_OAUTH2_CIBA_AUTH_CODE WITH : '+CAST(@batchCount as varchar);
			END
			...
			IF (@enableLog = 1)
			BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] BATCH DELETE FINISHED ON IDN_OAUTH2_CIBA_AUTH_CODE WITH : '+CAST(@deleteCount as varchar);
			END

			DELETE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_CODE_KEY in (select AUTH_CODE_KEY from BATCH_IDN_OAUTH2_CIBA_AUTH_CODE);

			IF (@enableLog = 1 AND @logLevel IN ('TRACE'))
			BEGIN
			SELECT '[' + convert(varchar, getdate(), 121) + '] DELETED BATCH ON CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE !';
			END
...
IF (@enableLog = 1)
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] CIBA AUTH CODE DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE COMPLETED .... !';
END
...
SELECT @rowcount = COUNT(1) FROM IDN_OAUTH2_CIBA_AUTH_CODE;
SELECT '[' + convert(varchar, getdate(), 121) + '] TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE AFTER DELETE :'+CAST(@rowCount as varchar);
END
