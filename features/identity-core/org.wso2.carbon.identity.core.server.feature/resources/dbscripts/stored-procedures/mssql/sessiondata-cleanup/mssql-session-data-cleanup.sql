...
SELECT @totalsession = @totalsession + COUNT(1) FROM IDN_AUTH_TEMP_SESSION_STORE where EXPIRY_TIME < @sessionCleanupTime;

SELECT 	'[' + convert(varchar, getdate(), 121) + '] TOTAL SESSION COUNT IN IDN_AUTH_SESSION_STORE & IDN_AUTH_TEMP_SESSION_STORE TO BE REMOVED' AS 'INFO LOG', @totalsession;
...
INSERT INTO IDN_AUTH_SESSION_STORE_TMP (SESSION_ID, SESSION_TYPE, TIME_CREATED) SELECT TOP (@chunkLimit) SESSION_ID, SESSION_TYPE, TIME_CREATED FROM IDN_AUTH_TEMP_SESSION_STORE where EXPIRY_TIME < @sessionCleanupTime;
...
DELETE A
FROM IDN_AUTH_TEMP_SESSION_STORE AS A
INNER JOIN TEMP_SESSION_BATCH AS B ON A.SESSION_ID = B.SESSION_ID AND A.SESSION_TYPE = B.SESSION_TYPE AND A.TIME_CREATED = B.TIME_CREATED;
SET @sessionCleanupCount = @sessionCleanupCount + @@ROWCOUNT;
...
IF (@tracingEnabled=1)
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] SESSION RECORDS REMOVED FROM IDN_AUTH_SESSION_STORE & IDN_AUTH_TEMP_SESSION_STORE: ' AS 'INFO LOG', @deletedSessions AS 'TOTAL NO OF DELETED ENTRIES';
...
INSERT INTO IDN_AUTH_SESSION_STORE_TMP (SESSION_ID,SESSION_TYPE) SELECT TOP (@chunkLimit) SESSION_ID,SESSION_TYPE FROM IDN_AUTH_TEMP_SESSION_STORE WHERE OPERATION = 'DELETE' AND TIME_CREATED < @operationCleanupTime;
...
INNER JOIN TEMP_SESSION_BATCH AS B
ON A.SESSION_ID = B.SESSION_ID AND A.SESSION_TYPE = B.SESSION_TYPE;;
SET @operationCleanupCount = @operationCleanupCount + @@ROWCOUNT;
...
IF (@tracingEnabled=1)
BEGIN
SELECT '[' + convert(varchar, getdate(), 121) + '] STORE OPERATION RECORDS REMOVED FROM IDN_AUTH_SESSION_STORE & IDN_AUTH_TEMP_SESSION_STORE: ' AS 'INFO LOG', @deletedStoreOperations  AS 'TOTAL NO OF DELETED STORE ENTRIES';
SELECT '[' + convert(varchar, getdate(), 121) + '] DELETE OPERATION RECORDS REMOVED FROM IDN_AUTH_SESSION_STORE & IDN_AUTH_TEMP_SESSION_STORE: ' AS 'INFO LOG', @deletedDeleteOperations AS 'TOTAL NO OF DELETED DELETE ENTRIES';
