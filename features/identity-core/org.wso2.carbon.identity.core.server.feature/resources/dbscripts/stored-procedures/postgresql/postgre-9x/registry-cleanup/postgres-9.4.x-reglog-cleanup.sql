CREATE OR REPLACE FUNCTION WSO2_REG_LOG_CLEANUP_9_4_x(IN operationid int) RETURNS TEXT AS $$
DECLARE
...
RAISE NOTICE 'INFO : WSO2_REG_LOG_CLEANUP_9_4_x() STARTED .... !';
...
THEN
    ...
    THEN
        ...
        THEN
            ...
            END IF;
            CREATE OR REPLACE VIEW REG_LOG_BACKUP_VIEW AS SELECT * FROM REG_LOG WHERE 1 = 2;
	    CREATE TABLE IF NOT EXISTS REG_LOG_BACKUP (LIKE REG_LOG_BACKUP_VIEW);
	    INSERT INTO REG_LOG_BACKUP SELECT * FROM REG_LOG_BACKUP_VIEW WHERE NOT EXISTS ( SELECT * FROM REG_LOG_BACKUP );
        ELSE
            IF (enableLog AND logLevel IN ('TRACE')) THEN
...
IF (operationid = 4)
    ...
    IF (enableLog)
    THEN
        RAISE NOTICE 'INFO : WSO2_REG_LOG_CLEANUP_9_4_x TASK COMPLETED .... !';
    END IF;
END IF;
