CREATE OR REPLACE FUNCTION WSO2_CONFIRMATION_CODE_CLEANUP_SP(IN operationid int) RETURNS TEXT AS $$
DECLARE
...
tablename  IN ('reg_resource_property', 'reg_property', 'reg_resource');
...
THEN
    purgingTable := 'reg_resource';
    purgeBseColmn := 'reg_version';
	purgeBseColmnType := 'int4';

    purgingCondition := 'select reg_version from reg_resource , reg_path where reg_resource.reg_path_id = reg_path.reg_path_id and reg_path.reg_path_value = ''/_system/config/repository/components/org.wso2.carbon.identity.mgt/data''
    and reg_version in (select reg_version from reg_resource_property , reg_property where reg_resource_property.reg_property_id = reg_property.reg_id and reg_property.reg_name = ''expiretime'' and (CAST(coalesce(REG_PROPERTY.REG_VALUE, ''0'') AS integer)/1000) < '||unix_timestamp||')';
...
THEN
    purgingTable := 'reg_resource_property';
    purgeBseColmn := 'reg_property_id';
    purgeBseColmnType := 'int4';

    purgingCondition := '
    select reg_property_id from reg_resource_property where reg_property_id not in (
...
ELSIF (operationid = 5 OR operationid = 6)
THEN
    purgingTable := 'reg_property';
    purgeBseColmn := 'reg_id';
    purgeBseColmnType := 'int4';

    purgingCondition := '
    select reg_id from reg_property a left join reg_resource_property b on a.reg_id = b.reg_property_id
...
THEN
    IF (enableLog) THEN
    RAISE NOTICE 'WSO2_CONFIRMATION_CODE_CLEANUP_SP STARTED .... !';
    RAISE NOTICE '';
    END IF;
