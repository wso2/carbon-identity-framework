...
THEN
    ...
    SELECT count(1) into ROWCOUNT FROM ALL_TABLES where OWNER = CURRENT_SCHEMA AND table_name = 'ADT_OAUTH2_CIBA_AUTH_CODE';
    IF (ROWCOUNT = 0)
    THEN
        IF (enableLog  AND logLevel IN ('TRACE') )
        THEN
            EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''CREATING AUDIT TABLE ADT_OAUTH2_CIBA_AUTH_CODE .. ! '')';
            COMMIT;
        END IF;
        EXECUTE IMMEDIATE 'CREATE TABLE ADT_OAUTH2_CIBA_AUTH_CODE as (SELECT * FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE 1 = 2)';
        COMMIT;
    ELSE
        IF (enableLog  AND logLevel IN ('TRACE'))
        THEN
            EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''USING AUDIT TABLE ADT_OAUTH2_CIBA_AUTH_CODE'')';
            COMMIT;
        END IF;
    END IF;
      IF (enableLog  AND logLevel IN ('TRACE'))
...
IF (enableLog)
THEN
    EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''CALCULATING CIBA AUTH CODES IN IDN_OAUTH2_CIBA_AUTH_CODE TABLE .... !'')';
    COMMIT;

    IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
    THEN
        SELECT COUNT(1) INTO ROWCOUNT FROM IDN_OAUTH2_CIBA_AUTH_CODE;
        EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE BEFORE DELETE : '||ROWCOUNT||''')';
        COMMIT;
    END IF;

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
        SELECT COUNT(1) INTO cleaupCount FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR
        (EXPIRES_IN > 0 AND deleteTimeLimit > (ISSUED_TIME + NUMTODSINTERVAL(EXPIRES_IN, 'SECOND')));
        EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL CIBA AUTH CODES SHOULD BE DELETED FROM IDN_OAUTH2_CIBA_AUTH_CODE : '||cleaupCount||''')';
        COMMIT;
    END IF;
    ...
    THEN
        ROWCOUNT := (ROWCOUNT - cleaupCount);
        EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL CIBA AUTH CODES SHOULD BE RETAIN IN IDN_OAUTH2_CIBA_AUTH_CODE : '||ROWCOUNT||''')';
        COMMIT;
    END IF;
END IF;
...
THEN
EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),'' '')';
EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''CIBA AUTH CODE DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE STARTED ... !'')';
COMMIT;
END IF;

LOOP
      SELECT COUNT(*) INTO ROWCOUNT from ALL_TABLES where OWNER = CURRENT_SCHEMA AND table_name = upper('CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE');
      IF (ROWCOUNT = 1) then
      EXECUTE IMMEDIATE 'DROP TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE';
      COMMIT;
      END if;

      EXECUTE IMMEDIATE 'CREATE TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE (ROW_ID rowid,CONSTRAINT CHNK_IDN_CIBA_AUTH_CODE_PRI PRIMARY KEY (ROW_ID)) NOLOGGING';
      COMMIT;

      EXECUTE IMMEDIATE 'INSERT /*+ APPEND */ INTO CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE (ROW_ID) SELECT rowid FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE rownum <= :chkSize AND (AUTH_REQ_STATUS IN (''EXPIRED'',''TOKEN_ISSUED'',''FAILED'') OR
      (EXPIRES_IN > 0 AND :dTL > (ISSUED_TIME + NUMTODSINTERVAL(EXPIRES_IN, ''SECOND''))))' using chunkSize,deleteTimeLimit;
      ...
      IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
      THEN
      EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''CHUNK TABLE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE CREATED WITH : '||chunkCount||''')';
      COMMIT;
      END IF;

      IF (enableAudit)
      THEN
      EXECUTE IMMEDIATE 'INSERT INTO ADT_OAUTH2_CIBA_AUTH_CODE SELECT CIBA.* FROM IDN_OAUTH2_CIBA_AUTH_CODE CIBA , CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE CHK WHERE CIBA.ROWID=CHK.ROW_ID';
      COMMIT;
      END IF;

      LOOP
          SELECT COUNT(*) INTO ROWCOUNT from ALL_TABLES where OWNER = CURRENT_SCHEMA AND table_name = upper('BATCH_IDN_OAUTH2_CIBA_AUTH_CODE');
          IF (ROWCOUNT = 1) then
          EXECUTE IMMEDIATE 'DROP TABLE BATCH_IDN_OAUTH2_CIBA_AUTH_CODE';
          COMMIT;
          END if;

          EXECUTE IMMEDIATE 'CREATE TABLE BATCH_IDN_OAUTH2_CIBA_AUTH_CODE (ROW_ID rowid,CONSTRAINT BATCH_IDN_CIBA_AUTH_CODE_PRI PRIMARY KEY (ROW_ID)) NOLOGGING';
          COMMIT;

          EXECUTE IMMEDIATE 'INSERT /*+ APPEND */ INTO BATCH_IDN_OAUTH2_CIBA_AUTH_CODE (ROW_ID) SELECT ROW_ID FROM CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE WHERE rownum <= '||batchSize||'';
          batchCount:= sql%rowcount;
          COMMIT;
          ...
          IF (enableLog AND logLevel IN ('TRACE'))
          THEN
          EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''BATCH DELETE START ON TABLE IDN_OAUTH2_CIBA_AUTH_CODE WITH : '||batchCount||''')';
          COMMIT;
          END IF;

          IF ((batchCount > 0))
          THEN
          EXECUTE IMMEDIATE 'DELETE FROM IDN_OAUTH2_CIBA_AUTH_CODE where rowid in (select ROW_ID from BATCH_IDN_OAUTH2_CIBA_AUTH_CODE)';
          deleteCount:= sql%rowcount;
          COMMIT;
          END IF;
          IF (enableLog)
          THEN
          EXECUTE IMMEDIATE  'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''BATCH DELETE FINISHED ON IDN_OAUTH2_CIBA_AUTH_CODE WITH : '||deleteCount||''')';
          COMMIT;
          END IF;

          EXECUTE IMMEDIATE 'DELETE CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE WHERE ROW_ID in (select ROW_ID from BATCH_IDN_OAUTH2_CIBA_AUTH_CODE)';
          COMMIT;
          IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
          THEN
          EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''DELETED BATCH ON  CHUNK_IDN_OAUTH2_CIBA_AUTH_CODE !'')';
          COMMIT;
          END IF;
...
IF (enableLog)
THEN
  EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''BATCH DELETE ON IDN_OAUTH2_CIBA_AUTH_CODE COMPLETED .... !'')';
  EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),'' '')';
  COMMIT;
END IF;
...
THEN
    ...
    SELECT COUNT(1) INTO ROWCOUNT FROM IDN_OAUTH2_CIBA_AUTH_CODE;
    EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_TOKEN_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL CIBA AUTH CODES ON IDN_OAUTH2_CIBA_AUTH_CODE TABLE AFTER DELETE :'||ROWCOUNT||''')';
    COMMIT;
