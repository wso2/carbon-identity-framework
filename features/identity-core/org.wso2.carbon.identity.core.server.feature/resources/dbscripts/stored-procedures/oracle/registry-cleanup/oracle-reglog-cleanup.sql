...
IF (enableLog)
THEN
    EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_REG_LOG_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''CALCULATING DELETE ELIGIBLE ON REG_LOG TABLE .... !'')';

    IF (enableLog AND logLevel IN ('DEBUG','TRACE'))
    THEN
    SELECT  COUNT(1) into rowcount FROM REG_LOG;
    EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_REG_LOG_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL REG_LOG TABLE BEFORE DELETE : '||rowCount||''')';
    END IF;

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
  SELECT COUNT(1) into cleanupCount FROM (
  ...
  (SELECT REG_LOG_ID FROM REG_LOG WHERE REG_ACTION = 7))A;
      EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_REG_LOG_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL REG_LOG SHOULD BE DELETED ON : '||cleanupCount||''')';
    END IF;

    IF (enableLog AND logLevel IN ('TRACE'))
    THEN
    rowcount  := (rowcount - cleanupCount);
          EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_REG_LOG_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''TOTAL REG_LOG SHOULD BE RETAIN : '||rowCount||''')';
    END IF;
END IF;
...
COMMIT;
        ...
        IF (enableLog AND logLevel IN ('TRACE'))
        THEN
        EXECUTE IMMEDIATE 'INSERT INTO LOG_WSO2_REG_LOG_CLEANUP_SP (TIMESTAMP,LOG) VALUES (TO_CHAR( SYSTIMESTAMP, ''DD.MM.YYYY HH24:MI:SS:FF4''),''BACKING UP TABLES REG_LOG_BACKUP'')';
        END IF;
