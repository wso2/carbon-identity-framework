...
-- ------------------------------------------------------
-- BACKUP IDN_OAUTH2_CIBA_AUTH_CODE TABLE
-- ------------------------------------------------------

    IF EXISTS (SELECT NAME FROM SYSIBM.SYSTABLES WHERE NAME = 'IDN_OAUTH2_CIBA_AUTH_CODE_BAK')
    THEN
        DROP TABLE IDN_OAUTH2_CIBA_AUTH_CODE_BAK;
    END IF;
    
    CREATE TABLE IDN_OAUTH2_CIBA_AUTH_CODE_BAK LIKE IDN_OAUTH2_CIBA_AUTH_CODE;
    INSERT INTO IDN_OAUTH2_CIBA_AUTH_CODE_BAK (SELECT * FROM IDN_OAUTH2_CIBA_AUTH_CODE);
END IF;
...
THEN
    ...
    IF NOT EXISTS (SELECT NAME FROM SYSIBM.SYSTABLES WHERE NAME = 'IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP_AUDITLOG')
    THEN
          CREATE TABLE IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP_AUDITLOG LIKE IDN_OAUTH2_CIBA_AUTH_CODE;
    END IF;
END IF;
...
IF enableAudit = 1
THEN
  INSERT INTO IDN_OAUTH2_CIBA_AUTH_CODE_CLEANUP_AUDITLOG SELECT * FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND deleteTillTime > (ISSUED_TIME + (EXPIRES_IN) SECONDS));
END IF;
...
THEN
    CALL dbms_alert.waitone('sleeping',message ,status,sleepTime);
END IF;
DELETE FROM IDN_OAUTH2_CIBA_AUTH_CODE WHERE AUTH_REQ_STATUS IN ('EXPIRED','TOKEN_ISSUED','FAILED') OR (EXPIRES_IN > 0 AND deleteTillTime > (ISSUED_TIME + (EXPIRES_IN) SECONDS)) LIMIT batchSize;
GET DIAGNOSTICS rowCount = ROW_COUNT;
UNTIL rowCount=0 END REPEAT;
