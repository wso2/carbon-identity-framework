...
BEGIN
	...
	SET @sessionCleanupTime = BIGINT(CURRENT TIMESTAMP - (1) HOUR );
	-- Operational data older than 60 minutes(1 h) will be removed.
	SET @operationCleanupTime = BIGINT(CURRENT TIMESTAMP - (1) HOUR);
	...
	WHILE (@operationCleanUpTempTableCount > 0) DO
		IF NOT (EXISTS (SELECT NAME FROM SYSIBM.SYSTABLES WHERE NAME= 'IDN_AUTH_SESSION_STORE_TMP2')) THEN
			...
			INSERT INTO IDN_AUTH_SESSION_STORE_TMP2 SELECT SESSION_ID, SESSION_TYPE FROM IDN_AUTH_SESSION_STORE WHERE OPERATION = 'DELETE' AND TIME_CREATED < @operationCleanupTime LIMIT @chunkLimit;
