...
BEGIN
	...
	WHILE (@sessionCleanUpTempTableCount > 0) DO
		IF NOT (EXISTS (SELECT NAME FROM SYSIBM.SYSTABLES WHERE NAME= 'IDN_AUTH_SESSION_STORE_TMP')) THEN
			...
			INSERT INTO IDN_AUTH_SESSION_STORE_TMP SELECT SESSION_ID FROM IDN_AUTH_TEMP_SESSION_STORE WHERE EXPIRY_TIME < @sessionCleanupTime LIMIT @chunkLimit;
		...
		WHILE (@sessionCleanupCount > 0) DO
			...
			DELETE FROM IDN_AUTH_TEMP_SESSION_STORE WHERE SESSION_ID IN (SELECT SESSION_ID FROM TEMP_SESSION_BATCH);
	...
	WHILE (@operationCleanUpTempTableCount > 0) DO
		IF NOT (EXISTS (SELECT NAME FROM SYSIBM.SYSTABLES WHERE NAME= 'IDN_AUTH_SESSION_STORE_TMP2')) THEN
			...
			INSERT INTO IDN_AUTH_SESSION_STORE_TMP2 SELECT SESSION_ID, SESSION_TYPE FROM IDN_AUTH_SESSION_STORE WHERE TIME_CREATED < @operationCleanupTime LIMIT @chunkLimit;
		 	INSERT INTO IDN_AUTH_SESSION_STORE_TMP2 SELECT SESSION_ID, SESSION_TYPE FROM IDN_AUTH_TEMP_SESSION_STORE WHERE TIME_CREATED < @operationCleanupTime LIMIT @chunkLimit;
		...
		WHILE (@operationCleanupCount > 0) DO
			...
			DELETE FROM IDN_AUTH_TEMP_SESSION_STORE WHERE EXISTS(SELECT 1 FROM TEMP_SESSION_BATCH2 WHERE IDN_AUTH_TEMP_SESSION_STORE.SESSION_ID = TEMP_SESSION_BATCH2.SESSION_ID AND IDN_AUTH_TEMP_SESSION_STORE.SESSION_TYPE = TEMP_SESSION_BATCH2.SESSION_TYPE);
