...
-- ------------------------------------------------------
  ...
  THEN
    ...
    IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'IDN_OAUTH2_ACCESS_TOKEN_BAK' AND
    TABLE_SCHEMA ='PUBLIC'))
    THEN
        DROP TABLE PUBLIC.IDN_OAUTH2_ACCESS_TOKEN_BAK;
    END IF;
...
-- ------------------------------------------------------
    ...
    THEN
        DROP TABLE PUBLIC.IDN_OAUTH2_AUTHORIZATION_CODE_BAK;
    END IF;
...
THEN
    ...
    THEN
          CREATE TABLE PUBLIC.IDN_OAUTH2_ACCESS_TOKEN_CLEANUP_AUDITLOG as SELECT * FROM PUBLIC.IDN_OAUTH2_ACCESS_TOKEN WHERE 1 = 2;
    END IF;
    ...
    THEN
          CREATE TABLE PUBLIC.IDN_OAUTH2_AUTHORIZATION_CODE_CLEANUP_AUDITLOG as SELECT * FROM PUBLIC.IDN_OAUTH2_AUTHORIZATION_CODE WHERE 1 = 2;
    END IF;
...
SELECT COUNT(*) INTO count from PUBLIC.IDN_OAUTH2_AUTHORIZATION_CODE acode WHERE EXISTS (SELECT * FROM PUBLIC.IDN_OAUTH2_ACCESS_TOKEN tok WHERE tok.TOKEN_ID = acode.TOKEN_ID) AND STATE IN ('ACTIVE') AND deleteTillTime < TIME_CREATED + INTERVAL '1minute' * (( VALIDITY_PERIOD / 1000 )/ 60 ) + INTERVAL '1hour' * timeIncrementFromGMT;

RAISE NOTICE 'TOTAL AUTHORIZATION CODES SHOULD BE RETAINED FROM IDN_OAUTH2_AUTHORIZATION_CODE: %', count;
