...
THEN
    ...
    THEN
        ...
        THEN
            IF (enableLog AND logLevel IN ('TRACE')) THEN
            RAISE NOTICE 'DROPPING TABLE CHUNK_IDN_OAUTH2_ACCESS_TOKEN HENCE COUNT IS %',chunkCount;
            END IF;
    ...
    THEN
        ...
        THEN
        RAISE NOTICE 'PROCEED WITH NEW CHUNK TABLE CHUNK_IDN_OAUTH2_ACCESS_TOKEN';
        END IF;
        ...
        ELSE
          IF (enableLog AND logLevel IN ('TRACE','DEBUG')) THEN
          RAISE NOTICE 'CHUNK TABLE chunk_idn_oauth2_access_token CREATED WITH : %',chunkCount;
          END IF;
          IF (enableAudit)
          THEN
    ...
    ELSE
        IF (enableLog AND logLevel IN ('TRACE')) THEN
        RAISE NOTICE 'PROCEED WITH EXISTING CHUNK TABLE chunk_idn_oauth2_access_token WITH %',chunkCount;
        END IF;
    END IF;
...
THEN
      ...
      IF (rowcount = 1)
      THEN
          SELECT COUNT(1) INTO chunkCount FROM idn_oauth2_authorization_code_chunk;
          IF (chunkCount = 0)
          THEN
              IF (enableLog AND logLevel IN ('TRACE')) THEN
              RAISE NOTICE 'DROPPING TABLE IDN_OAUTH2_AUTHORIZATION_CODE_CHUNK HENCE COUNT IS %',chunkCount;
              END IF;
      ...
      THEN
          IF (enableLog AND logLevel IN ('TRACE'))
          THEN
          RAISE NOTICE 'PROCEED WITH NEW CHUNK TABLE IDN_OAUTH2_AUTHORIZATION_CODE_CHUNK';
          END IF;
          ...
          ELSE
              IF (enableLog AND logLevel IN ('TRACE','DEBUG')) THEN
              RAISE NOTICE 'CHUNK TABLE IDN_OAUTH2_AUTHORIZATION_CODE_CHUNK CREATED WITH : %',chunkCount;
              END IF;
