...
CREATE TABLE IF NOT EXISTS IDN_PD_CREDENTIAL (
    ...
    CONSTRAINT FK_IDN_PD_CREDENTIAL_DEFINITION FOREIGN KEY (DEFINITION_ID) REFERENCES IDN_PRESENTATION_DEFINITION(DEFINITION_ID)
)ENGINE NDB;

-- NDB does not support ON DELETE CASCADE when the child table contains TEXT/BLOB columns.
-- This trigger deletes child credentials before the parent definition is removed.
CREATE TRIGGER trg_delete_pd_credentials
BEFORE DELETE ON IDN_PRESENTATION_DEFINITION
FOR EACH ROW
DELETE FROM IDN_PD_CREDENTIAL WHERE DEFINITION_ID = OLD.DEFINITION_ID;
