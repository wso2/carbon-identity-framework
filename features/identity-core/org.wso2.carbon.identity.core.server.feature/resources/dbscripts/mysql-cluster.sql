...
CREATE TABLE IF NOT EXISTS IDN_PRESENTATION_DEFINITION (
    ...
    TENANT_ID     INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    KEY IDN_PD_CURSOR_KEY (CURSOR_KEY),
...
CREATE TABLE IF NOT EXISTS IDN_PD_CREDENTIAL (
    ...
    CONSTRAINT FK_IDN_PD_CREDENTIAL_DEFINITION FOREIGN KEY (DEFINITION_ID) REFERENCES IDN_PRESENTATION_DEFINITION(ID)
)ENGINE NDB;
...
CREATE TABLE IF NOT EXISTS IDN_PD_CLAIM (
    ...
    TENANT_ID       INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT FK_IDN_PD_CLAIM_CREDENTIAL FOREIGN KEY (CREDENTIAL_ID) REFERENCES IDN_PD_CREDENTIAL(ID)
)ENGINE NDB;

-- NDB does not support ON DELETE CASCADE when the child table contains TEXT/BLOB columns.
-- These triggers delete child rows before the parent is removed.
CREATE TABLE IF NOT EXISTS IDN_PD_ISSUER_CONFIG (
    ...
    TENANT_ID             INTEGER       NOT NULL,
    PRIMARY KEY (ID),
    CONSTRAINT FK_IDN_PD_ISSUER_CONFIG_CREDENTIAL FOREIGN KEY (CREDENTIAL_ID) REFERENCES IDN_PD_CREDENTIAL(ID)
)ENGINE NDB;

CREATE TRIGGER trg_delete_pd_claims
BEFORE DELETE ON IDN_PD_CREDENTIAL
FOR EACH ROW
DELETE FROM IDN_PD_CLAIM WHERE CREDENTIAL_ID = OLD.ID;

CREATE TRIGGER trg_delete_pd_issuer_configs
BEFORE DELETE ON IDN_PD_CREDENTIAL
FOR EACH ROW
DELETE FROM IDN_PD_ISSUER_CONFIG WHERE CREDENTIAL_ID = OLD.ID;

CREATE TRIGGER trg_delete_pd_credentials
BEFORE DELETE ON IDN_PRESENTATION_DEFINITION
FOR EACH ROW
DELETE FROM IDN_PD_CREDENTIAL WHERE DEFINITION_ID = OLD.ID;
